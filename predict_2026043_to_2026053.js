/**
 * 以2026043为源行，间隔10预测2026053期 - V4模式（间隔9+10并集池策略）
 * 
 * 方向修正：
 *   源行 = 2026043（用户选中）
 *   目标 = 2026043 + 10 = 2026053
 *   间隔9源期 = 2026053 - 9 = 2026044
 *   间隔10源期 = 2026053 - 10 = 2026043
 */
...
console.log("╔══════════════════════════════════════════════════════════════════════╗");
console.log("║  🎯 源行2026043 → 目标2026053（间隔10）V4并集池策略                 ║");
console.log("╚══════════════════════════════════════════════════════════════════════╝\n");

// 方向：源行2026043，间隔10 → 目标2026053
const sourceRow = '2026043';
const targetIssue = '2026053';  // 2026043 + 10 = 2026053
const targetNum = parseInt(targetIssue.slice(4));

// 目标期的两个回溯源期
const source9Issue = targetIssue.slice(0, 4) + String(targetNum - 9).padStart(3, '0');   // 2026044
const source10Issue = targetIssue.slice(0, 4) + String(targetNum - 10).padStart(3, '0'); // 2026043

console.log(`📍 用户选中源行：${sourceRow}`);
console.log(`🎯 目标期：${targetIssue}（源行 + 间隔10）`);
console.log(`📊 间隔9源期：${source9Issue}（目标-9）`);
console.log(`📊 间隔10源期：${source10Issue}（目标-10 = 用户源行）\n`);
...
console.log(`\n📦 号码池统计：`);
console.log(`   间隔9池(${source9Issue})：${pool9.size}个`);
console.log(`   间隔10池(${source10Issue})：${pool10.size}个`);
...
// ===== Top5组合生成 =====
console.log(`\n${'═'.repeat(70)}`);
console.log("🏆 Top1组合（并集池综合分数最高号码）");
console.log('═'.repeat(70));
...
console.log(`\n🔗 联合Top5覆盖（${source9Issue}+${source10Issue}各5组去重）：`);
...
if (targetDraw) {
  ...
  console.log(`   实际命中：${jointHit.length}/5 → [${jointHit.join(', ')}]`);
  ...
  top5_10.forEach((combo, i) => {
    const hits = combo.numbers.filter(n => targetSet.has(n));
    comboHits.push({ label: `间隔10(源行${sourceRow})-Top${i+1}`, numbers: combo.numbers, hits: hits.length, hitNums: hits });
  });
...
// ===== 补漏6（补盲区策略） =====
console.log(`\n${'═'.repeat(70)}`);
console.log("🔍 补漏6（补盲区策略 - 覆盖Top1未覆盖的并集池号码）");
console.log('═'.repeat(70));
...
if (backResult) {
  ...
  if (targetDraw) {
    ...
    console.log(`   实际命中(目标期后区 [${targetDraw.back.join(', ')}])：${backHit.length}/2 → [${backHit.join(', ')}]${backHit.length >= 1 ? ' ✓' : ' ✗'}`);
  }
}
...
console.log(`
🎯 前区推荐（源行${sourceRow}→目标${targetIssue} V4并集池）：
   源行：${sourceRow} [${source10Draw.front.join(', ')}]
   目标：${targetIssue} [${targetDraw.front.join(', ')}]
   主组合：[${top5Numbers.join(', ')}]
   补漏6：[${bulou6.join(', ')}]
   联合覆盖：${allSelected.size}个号码
   
📊 策略配置：
   - 方向：源行${sourceRow} → 间隔10 → 目标${targetIssue}
   - 趋势映射：间隔9(${source9Issue})+10(${source10Issue})并集池（权重0.7:0.3）
...
console.log(`
指标                    | 间隔9(${source9Issue}) | 间隔10(${source10Issue}) | 并集池   
------------------------|----------|----------|----------
号码池大小              | ${String(pool9.size).padStart(8)} | ${String(pool10.size).padStart(8)} | ${String(unionPool.size).padStart(8)}
命中目标期(${targetIssue})  | ${String(pool9HitCompare.length).padStart(6)}/5  | ${String(pool10HitCompare.length).padStart(6)}/5  | ${String(unionHitCompare.length).padStart(6)}/5
