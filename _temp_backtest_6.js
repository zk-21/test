...
function buildV4PlusTenTrendMap(sourceRow, sourceNumbers, boardBalls) {
  ...
  const weights = [0.2, 0.25, 0.3, 0.25]; // 间隔9权重更高（回测最优配置）
...
if (__isNode) {
  ...
  const PREDICT_INTERVAL = 7; // 预测间隔10后的期数
  const SECOND_INTERVAL = 8;   // 第二间隔9（用于并集池合并）
  ...
  for (let sourceIdx = 1; sourceIdx <= totalDraws - PREDICT_INTERVAL - 1; sourceIdx++) {
    ...
    try {
      ...
      // 间隔8的源
      const auxSource1Idx = sourceIdx + 7;
      ...
      // 间隔9的源
      const auxSource2Idx = sourceIdx + 8;
      ...
      // 间隔10的源
      const auxSource3Idx = sourceIdx + 9;
      const auxSource3Row = auxSource3Idx + 1;
      if (auxSource3Row >= 1 && auxSource3Row <= totalDraws && auxSource3Row !== sourceRow) {
        sourceRows.push(auxSource3Row);
      }
