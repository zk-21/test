/**
 * backtest_v5_nopair.js — 对比：6维配对 vs 6维独立（不配对）
 * 
 * A) 原版配对：tail×0.55+offset×0.45 →3注, hot×0.50+freq×0.50 →3注, bridge×0.50+arith×0.50 →2注
 * B) 独立不配对：tail→2注, offset→2注, hot→1注, freq→1注, bridge→1注, arith→1注
 * 
 * 运行：node backtest_v5_nopair.js
 */
...
// ======================== 12. 方案B：6维独立（不配对） ========================
function buildCombosIndependent(dimData, sourceNums, predictedTails) {
  const { nTail, nOff, nHot, nFreq, nBr, nAr } = dimData;
...
function backtestBoth(sourceIdx, allDraws) {
  ...
  // B: 独立
  const combosB = buildCombosIndependent(dimData, sourceNums, predictedTails);
...
console.log('对比测试：6维配对(A) vs 6维独立不配对(B)');
console.log('='.repeat(100));
...
console.log('='.repeat(100));
console.log('');
console.log('指标'.padEnd(25) + 'A)配对'.padEnd(15) + 'B)独立'.padEnd(15) + '差异'.padEnd(12) + '胜者');
console.log('-'.repeat(67));
console.log('Top5平均最高命中'.padEnd(25) + `${(sumA_top5/cnt).toFixed(2)}/5`.padEnd(15) + `${(sumB_top5/cnt).toFixed(2)}/5`.padEnd(15) + `${(sumB_top5-sumA_top5>=0?'+':'')+(sumB_top5/cnt-sumA_top5/cnt).toFixed(2)}`.padEnd(12) + (sumB_top5>=sumA_top5?'B':'A'));
console.log('Top5联合覆盖率'.padEnd(25) + `${(sumA_union/cnt).toFixed(2)}/5`.padEnd(15) + `${(sumB_union/cnt).toFixed(2)}/5`.padEnd(15) + `${(sumB_union-sumA_union>=0?'+':'')+(sumB_union/cnt-sumA_union/cnt).toFixed(2)}`.padEnd(12) + (sumB_union>=sumA_union?'B':'A'));
console.log('补漏6平均命中'.padEnd(25) + `${(sumA_bl6/cnt).toFixed(2)}/5`.padEnd(15) + `${(sumB_bl6/cnt).toFixed(2)}/5`.padEnd(15) + `${(sumB_bl6-sumA_bl6>=0?'+':'')+(sumB_bl6/cnt-sumA_bl6/cnt).toFixed(2)}`.padEnd(12) + (sumB_bl6>=sumA_bl6?'B':'A'));
console.log('补漏6后总覆盖率'.padEnd(25) + `${(sumA_total/cnt).toFixed(2)}/5`.padEnd(15) + `${(sumB_total/cnt).toFixed(2)}/5`.padEnd(15) + `${(sumB_total-sumA_total>=0?'+':'')+(sumB_total/cnt-sumA_total/cnt).toFixed(2)}`.padEnd(12) + (sumB_total>=sumA_total?'B':'A'));
console.log('候选池覆盖'.padEnd(25) + `${(sumA_pool/cnt).toFixed(2)}/5`.padEnd(15) + `${(sumB_pool/cnt).toFixed(2)}/5`.padEnd(15) + '相同'.padEnd(12) + '—');
...
for (let h = 5; h >= 0; h--) {
  ...
  if (a > 0 || b > 0) {
    ...
    console.log(`  ${h}个    | ${String(a).padEnd(6)} | ${String(b).padEnd(6)} | ${diffStr}`);
  }
}
