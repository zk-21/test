...
function buildSampleNumbersV4(selectedRow, zone, ratios, _preCollected) {
  ...
  for (let n = 1; n <= 35; n++) {
    ...
    if (tailCorrelationBonus > 0) {
      score += Math.round(tailCorrelationBonus * 1.0);
    }
    ...
    if (n === 35) {
      ...
      if (hasZone1Anchor) { score += 10; specialTriggered = true; }
      score += 3;
    }
    if (n === 30) {
      if (selectedNumbers.includes(29)) { score += 12; specialTriggered = true; }
      if (selectedNumbers.includes(28) || selectedNumbers.includes(31)) { score += 6; specialTriggered = true; }
    }
    if (n === 31) {
      if (selectedNumbers.includes(30) || selectedNumbers.includes(32)) { score += 8; specialTriggered = true; }
    }
    if (n === 25) {
      if (selectedNumbers.includes(24) || selectedNumbers.includes(26)) { score += 7; specialTriggered = true; }
    }
    if (n === 26) {
      if (selectedNumbers.includes(25) || selectedNumbers.includes(27)) { score += 6; specialTriggered = true; }
      if (selectedNumbers.some(x => x >= 15 && x <= 20)) score += 3;
    }
    if (n === 27) {
      if (selectedNumbers.includes(26) || selectedNumbers.includes(28)) { score += 5; specialTriggered = true; }
    }
...
if (__isNode) {
  ...
  for (let sourceIdx = 1; sourceIdx <= totalDraws - PREDICT_INTERVAL - 1; sourceIdx++) {
    ...
    try {
      ...
      frontSample.ivPrediction = predictTargetIntervalRatio(targetIdx, predSourceIv, __allBalls);
      ...
      // 加权合并（主源0.5，辅源0.3，第三源0.2）
      const weights = [0.5, 0.3, 0.2];
      const scoreMap = new Map();
      // 🆕 诊断：记录每个源池的完整35球排名（不仅是Top30）
      const _diagSourceAll35 = allSamples.map(item => {
        const all = (item.sample._allCandidates || []).map((c, i) => ({ num: c.number, score: c.score }));
        // all已按分数降序排列
        return all;
      });
      const _diagSourcePoolSets = _diagSourceAll35.map(all => new Set(all.map(e => e.num)));
      ...
      // 🆕 补充池：添加在所有3个源中都排名31-35的系统性低估球
      {
        const mainPoolNums = new Set(mergedEntries.map(e => e.number));
        const supplementEntries = [];
        // 遍历所有35球（1-35），找出在主池中缺失但在所有源35球中都存在的球
        for (let num = 1; num <= 35; num++) {
          if (mainPoolNums.has(num)) continue;
          if (supplementEntries.length >= 3) break;
          // 检查是否在所有3个源的完整35球中都存在但都不在Top30
          const srcRanks = _diagSourceAll35.map(all => {
            const idx = all.findIndex(e => e.num === num);
            return idx >= 0 ? idx + 1 : 0;
          });
          const allPresent = srcRanks.every(r => r > 0);
          const allBelow30 = srcRanks.every(r => r > V4_POOL_SIZE);
          if (allPresent && allBelow30) {
            // 计算融合分数
            let suppScore = 0;
            allSamples.forEach((item, idx) => {
              const w = weights[idx] || 0.1;
              const found = (item.sample._allCandidates || []).find(e => e.number === num);
              if (found) suppScore += found.score * w;
            });
            let o = {};
            for (const item of allSamples) {
              const found = (item.sample._allCandidates || []).find(e => e.number === num);
            ...
            supplementEntries.push({
              number: num, score: suppScore, baseScore: suppScore, rank: 0,
              selectedTailHits: o.selectedTailHits || 0, selectedTailNeighborHits: o.selectedTailNeighborHits || 0,
              tailCount: o.tailCount || 0, lastRowTailHits: o.lastRowTailHits || 0,
              tailPatternScore: o.tailPatternScore || 0, upperColorHits: o.upperColorHits || 0,
              upperColorTailHits: o.upperColorTailHits || 0, upperColorTailNeighborHits: o.upperColorTailNeighborHits || 0,
        ...
        if (supplementEntries.length > 0) {
          mergedEntries.push(...supplementEntries);
        }
      }
      ...
      // 4. 首位球预测（复用主源结果）
      const firstBallPreds = frontSample.firstBallPredictions || null;
      
      // 5. V5 组合生成（8维度评分 + 随机组合 + 重评分）
      ...
      const frontCombos = buildSampleFrontCombosV5(
        ...
        ratioPlan, sourceRow, __allBalls
      );
