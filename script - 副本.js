...
function analyzeTailCorrelation(allBalls, sourceRow, lookback = 100) {
  ...
  const mixedPatternFreq = new Map(); // 混合模式（连续+等差）
    ...
    for (let i = 0; i < drawTails.length; i++) {
      for (let j = i + 1; j < drawTails.length; j++) {
        for (let k = j + 1; k < drawTails.length; k++) {
          ...
          const isArithmeticPair = (Math.abs(t2 - t1) === 2 || Math.abs(t2 - t1) === 8); // 步长2
...
function predictLikelyTailsV4Enhanced(sourceTails, transData, refRows, sourceRow, allBalls = null) {
  ...
  const weights = {
    overlap1: 6,      // 原15→6：降低重叠权重
    arith1: 12,       // 原6→12：提升等差权重（97.5%出现率）
    overlap10: 4,     // 原12→4：降低10期重叠权重
    arith10: 10,      // 原4→10：提升10期等差权重
    overlapBonus: 2,  // 原10→2：降低重叠奖励
    globalFreq: 28,   // 新增：全局高频尾号权重
  };
...
const V4_POOL_SIZE = 24; // 24球池：覆盖率98.2%，Top5=26.3%，效率最高
  ...
  if (candidateEntries && candidateEntries.length > 0 && selectedFront.length >= 1) {
    ...
    const c6Scored = candidateEntries
      ...
      .map(e => {
        ...
        let s6 = e.score || 0;
        // 尾号匹配（移植 optimized_picker）
        if (predTails6.has(n % 10)) s6 += 10;
...
sampleButton.addEventListener("click", () => {
  ...
  const frontCombos = isV4
    ? buildSampleFrontCombosV4(frontSample.candidateEntries, v4Refs, frontSample.selectedNumbers, frontSample.selectedNumbers, frontSample.predictedTails || null, frontSample.ivPrediction || null, firstBallPreds, frontSample.extremeFlags || null, ratioPlan)
