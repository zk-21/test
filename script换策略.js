...
function predictLikelyTailsV4Enhanced(sourceTails, transData, refRows, sourceRow, allBalls = null) {
  // 🆕 优化权重（基于179期回测：85.6%的号码尾号与前一期相同或相邻）
  const weights = {
    overlap1: ABLATION_CONFIG.disable_overlap1 ? 0 : 6,      // 降低：改为使用新的sameOrNeighbor权重
    arith1: ABLATION_CONFIG.disable_arith1 ? 0 : 8,        // 降低：等差模式只有6.7%
    overlap10: 2,     // 降低：远期参考影响较小
    arith10: 6,       // 降低：等差模式只有6.7%
    overlapBonus: 2,  // 保持：双重保留更可靠
    globalFreq: 20,   // 降低：为新权重腾出空间
    comboTransfer: ABLATION_CONFIG.disable_comboTransfer ? 0 : 2, // 大幅降低：完全匹配率0%
    partialMatch: 10, // 降低：为新权重腾出空间
    highFreqMiss: 8,  // 降低：为新权重腾出空间
    sameOrNeighbor: ABLATION_CONFIG.disable_sameOrNeighbor ? 0 : 35, // 核心权重：相同或相邻尾号（85.6%概率）
    crossRowTail: ABLATION_CONFIG.disable_crossRowTail ? 0 : 15,   // 🆕 跨行尾号：与前2-3期尾号重复
    intraGroupPattern: ABLATION_CONFIG.disable_intraGroupPattern ? 0 : 18, // 🆕 组内尾号规则：历史尾号组合模式（12→18优化）
    arithmeticNumber: ABLATION_CONFIG.disable_arithmeticNumber ? 0 : 10, // 🆕 完整号码等差：号码1-35的等差延伸
    crossRowArithmetic: ABLATION_CONFIG.disable_crossRowArithmetic ? 0 : 24, // 🆕 跨行等差：上一期与当前期号码的等差延伸（12→24优化）
    bridgeTails: ABLATION_CONFIG.disable_bridgeTails ? 0 : 12,        // 🆕 桥接尾号：两个尾号之间的中间尾号（提升权重）
  };
...
function analyzeCrossRowTails(sourceRow, allBalls) {
  ...
  for (let r = start; r < sourceRow - 2; r++) {
    const srcNums = [...new Set(ballsByRow.get(r) || [])];
    const tgtNums = [...new Set(ballsByRow.get(r + 1) || [])];
    ...
    // 检查与前2-3期的尾号重复
    for (let lookback = 2; lookback <= 3; lookback++) {
      const prevRow = r - lookback;
      const prevNums = [...new Set(ballsByRow.get(prevRow) || [])];
      if (prevNums.length !== 5) continue;
      
      const prevTails = new Set(prevNums.map(n => n % 10));
      
      // 统计目标期尾号与前2-3期尾号的重复
      tgtTails.forEach(tail => {
        if (prevTails.has(tail)) {
  ...
  // 获取前2-3期的尾号，给予额外加分
  if (sourceRow > 2) {
    const ref2 = ballsByRow.get(sourceRow - 2);
    const ref3 = ballsByRow.get(sourceRow - 3);
    
    if (ref2 && ref2.length === 5) {
      const ref2Tails = new Set(ref2.map(n => n % 10));
      ref2Tails.forEach(tail => {
        tailScores.set(tail, tailScores.get(tail) + 2); // 前2期加分
      });
    }
    
    if (ref3 && ref3.length === 5) {
      const ref3Tails = new Set(ref3.map(n => n % 10));
      ref3Tails.forEach(tail => {
        tailScores.set(tail, tailScores.get(tail) + 1); // 前3期加分
      });
    }
  }
...
function analyzeIntraGroupTailPatterns(sourceRow, allBalls) {
  ...
  // 统计尾号组合模式
  const diversityFreq = new Map(); // 尾号多样性（不同尾号个数）
  const consecutiveFreq = new Map(); // 连续尾号频率
  const arithmeticFreq = new Map(); // 等差尾号频率
  ...
  diversityFreq.forEach((count, tail) => {
    const score = (count / maxDiversity) * 4; // 多样性权重4
    tailScores.set(tail, tailScores.get(tail) + score);
  });
  
  consecutiveFreq.forEach((count, tail) => {
    const score = (count / maxConsecutive) * 3; // 连续性权重3
    tailScores.set(tail, tailScores.get(tail) + score);
  });
  
  arithmeticFreq.forEach((count, tail) => {
    const score = (count / maxArithmetic) * 3; // 等差性权重3
    tailScores.set(tail, tailScores.get(tail) + score);
  });
...
function buildSampleNumbersV4(selectedRow, zone, ratios, _preCollected) {
  ...
  if (predictedTails && predictedTails.length > 0) {
    ...
    predictedTails.slice(0, 5).forEach(([tail]) => {
      predictedTailSet.add((tail + 1) % 10);
      predictedTailSet.add((tail + 9) % 10); // tail - 1
    });
  }
...
function buildSampleNumbersV4TailOnly(selectedRow, zone, ratios, _preCollected) {
  ...
  // ═══ 只使用预测尾号+邻号构建候选池 ═══
  const predictedTailSet = new Set();
  const TAIL_PREDICT_TOP_N = 5; // 🆕 恢复Top5预测尾号（Top6测试效果更差）
  if (predictedTails && predictedTails.length > 0) {
    // 取TopN预测尾号
    predictedTails.slice(0, TAIL_PREDICT_TOP_N).forEach(([tail]) => predictedTailSet.add(tail));
    // 也加入邻号尾号（±1）
    predictedTails.slice(0, TAIL_PREDICT_TOP_N).forEach(([tail]) => {
      predictedTailSet.add((tail + 1) % 10);
      predictedTailSet.add((tail + 9) % 10); // tail - 1
    });
  }
  ...
  for (let n = 1; n <= 35; n++) {
    const t = n % 10;
    if (predictedTailSet.has(t)) {
      candidates.push({ number: n, score: 1, signals: ['tail_match'] });
    }
  }
  ...
  for (const n of selectedNumbers) {
    if (!candidates.some(c => c.number === n)) {
      candidates.push({ number: n, score: 1, signals: ['source_number'] });
    }
  }
...
// ═══ 策略驱动的组合生成 V2（增加多样性：每策略限3注 + 补充V5多样性组合） ═══
function buildStrategyDrivenCombos(entries, refs, anchorNumbers, selectedNumbers, predictTails, ivPrediction, firstBallPredictions, extremeFlags, manualRatio, sourceRow, _preCollected) {
  ...
  var predictedIv = srcIv.map(function (c, idx) {
    if (c >= 4) return Math.max(1, c - 2);
    if (c === 0) return 2;
    if (c > prevIv[idx]) return Math.max(1, c - 1);
    if (c < prevIv[idx]) return c + 1;
    return c;
  });
  ...
  entries.forEach(function (e) { entryMap.set(e.number, e); });
  var allEntries = entries.slice().sort(function (a, b) { return b.score - a.score; });

  // ========== 3. 辅助函数 ==========
  function isInPredictedInterval(n) {
    var zone = n <= 12 ? 0 : (n <= 24 ? 1 : 2);
    return predictedIv[zone] > 0;
  }
  function hasArithPartner(n) {
    return entries.some(function (other) { var d = Math.abs(n - other.number); return d >= 2 && d <= 8; });
  }
  function isSynergyNumber(n) {
    return predictedTailSet.has(n % 10) && isInPredictedInterval(n) && hasArithPartner(n);
  }
  function checkArithPair(nums) {
  ...
  function comboKey(nums) { return nums.slice().sort(function (a, b) { return a - b; }).join(','); }

  var usedKeys = new Set();
  var allCombos = [];
  // 限制每个策略最多生成数量，保证多样性
  var STRATEGY_LIMIT = 3;

  function pushCombo(nums, strategy) {
    nums = nums.slice().sort(function (a, b) { return a - b; });
    if (nums.length !== 5) return;
    var key = comboKey(nums);
    if (usedKeys.has(key)) return;
    usedKeys.add(key);
    var score = nums.reduce(function (s, n) { var e = entryMap.get(n); return s + (e ? e.score : 0); }, 0);
    allCombos.push({
      numbers: nums, key: key, score: score, strategy: strategy,
      tailCount: nums.filter(function (n) { return predictedTailSet.has(n % 10); }).length,
      anchorCount: nums.filter(function (n) { return anchorNumbers.indexOf(n) >= 0; }).length,
      synergyCount: nums.filter(function (n) { return isSynergyNumber(n); }).length,
      hasArithPair: checkArithPair(nums),
    });
  }

  // ========== 4. 策略1: 三重协同 ==========
  var synergyNumbers = entries.filter(function (e) { return isSynergyNumber(e.number); })
    .sort(function (a, b) { return b.score - a.score; });
  var synCount = 0;
  if (synergyNumbers.length >= 2) {
    for (var i = 0; i < Math.min(5, synergyNumbers.length) && synCount < STRATEGY_LIMIT; i++) {
      for (var j = i + 1; j < Math.min(8, synergyNumbers.length) && synCount < STRATEGY_LIMIT; j++) {
        var pair = [synergyNumbers[i].number, synergyNumbers[j].number];
        // 补充时选择覆盖不同区间的号码
        var remaining = allEntries.filter(function (e) {
          return pair.indexOf(e.number) < 0;
        });
        // 多样化：用不同种子选择不同的补充号码
        var seed = (i * 7 + j * 13) % remaining.length;
        var combo = pair.slice();
        for (var r = 0; r < remaining.length && combo.length < 5; r++) {
          var idx = (seed + r) % remaining.length;
          if (combo.indexOf(remaining[idx].number) < 0) combo.push(remaining[idx].number);
        }
        pushCombo(combo, 'synergy');
        synCount++;
      }
    }
  }

  // ========== 5. 策略2: 区间匹配（只取3个代表性组合） ==========
  var z1Count = Math.max(1, Math.min(3, predictedIv[0]));
  var z2Count = Math.max(1, Math.min(3, predictedIv[1]));
  var z3Count = Math.max(1, Math.min(3, predictedIv[2]));
  ...
  var z1 = entries.filter(function (e) { return e.number <= 12; }).sort(function (a, b) { return b.score - a.score; });
  var z2 = entries.filter(function (e) { return e.number >= 13 && e.number <= 24; }).sort(function (a, b) { return b.score - a.score; });
  var z3 = entries.filter(function (e) { return e.number >= 25; }).sort(function (a, b) { return b.score - a.score; });

  var ivCount = 0;
  for (var i1 = 0; i1 < Math.min(z1Count + 1, z1.length) && ivCount < STRATEGY_LIMIT; i1++) {
    for (var i2 = 0; i2 < Math.min(z2Count + 1, z2.length) && ivCount < STRATEGY_LIMIT; i2++) {
      for (var i3 = 0; i3 < Math.min(z3Count + 1, z3.length) && ivCount < STRATEGY_LIMIT; i3++) {
        var partial = [z1[i1].number, z2[i2].number, z3[i3].number];
        // 从剩余高分候选中补充
        var remaining = allEntries.filter(function (e) { return partial.indexOf(e.number) < 0; });
        for (var r = 0; r < remaining.length && partial.length < 5; r++) {
          partial.push(remaining[r].number);
        }
        pushCombo(partial, 'interval_match');
        ivCount++;
      }
    }
  }
  ...
  // ========== 6. 策略3: 尾号匹配 ==========
  var tailNumbers = entries.filter(function (e) { return predictedTailSet.has(e.number % 10); })
    .sort(function (a, b) { return b.score - a.score; });
  var tCount = 0;
  for (var i = 0; i < Math.min(6, tailNumbers.length) && tCount < STRATEGY_LIMIT; i++) {
    for (var j = i + 1; j < Math.min(8, tailNumbers.length) && tCount < STRATEGY_LIMIT; j++) {
      var pair = [tailNumbers[i].number, tailNumbers[j].number];
      var remaining = allEntries.filter(function (e) { return pair.indexOf(e.number) < 0; });
      var combo = pair.concat(remaining.slice(0, 3).map(function (e) { return e.number; }));
      pushCombo(combo, 'tail_match');
      tCount++;
    }
  }
  ...
  for (var i = 0; i < entries.length; i++) {
    for (var j = i + 1; j < entries.length; j++) {
      ...
      if (diff >= 2 && diff <= 8) arithPairs.push([entries[i], entries[j], diff]);
    }
  }
  arithPairs.sort(function (a, b) { return (b[0].score + b[1].score) - (a[0].score + a[1].score); });
  var aCount = 0;
  for (var p = 0; p < Math.min(10, arithPairs.length) && aCount < STRATEGY_LIMIT; p++) {
    var pair = [arithPairs[p][0].number, arithPairs[p][1].number];
    var remaining = allEntries.filter(function (e) { return pair.indexOf(e.number) < 0; });
    var combo = pair.concat(remaining.slice(0, 3).map(function (e) { return e.number; }));
    pushCombo(combo, 'arithmetic');
    aCount++;
  }

  // ========== 8. 补充多样性组合（从6维度评分中取不同组合） ==========
  // 尾号维度 Top3 组合
  var tailPool = allEntries.filter(function (e) { return predictedTailSet.has(e.number % 10); }).slice(0, 10);
  var offsetPool = allEntries.slice(0, 10);
  var anchorPool = anchorNumbers.length > 0
    ? allEntries.filter(function (e) { return anchorNumbers.indexOf(e.number) >= 0; }).slice(0, 8)
    : [];

  // 生成多样性补充组合
  var divPools = [
    { pool: allEntries.slice(0, 12), label: 'score_top' },
    { pool: tailPool.length >= 5 ? tailPool : allEntries.slice(0, 10), label: 'tail_div' },
    { pool: anchorPool.length >= 3 ? anchorPool.concat(allEntries.slice(0, 8)) : allEntries.slice(0, 10), label: 'anchor_div' },
  ];

  divPools.forEach(function (dp) {
    var pool = dp.pool;
    if (pool.length < 5) return;
    // 生成不重叠的3个组合
    for (var start = 0; start < Math.min(9, pool.length - 4); start += 3) {
      var nums = [];
      for (var k = start; k < Math.min(start + 5, pool.length); k++) nums.push(pool[k].number);
      if (nums.length === 5) pushCombo(nums, dp.label);
    }
  });

  // ========== 9. 排序：策略优先 + 多样性 ==========
  var strategyPriority = { 'synergy': 6, 'interval_match': 5, 'tail_match': 4, 'arithmetic': 3, 'score_top': 2, 'tail_div': 1, 'anchor_div': 1 };
  ...
  // 去重（重叠≥3跳过）
  var finalCombo = [], fk = new Set();
  for (var fi = 0; fi < allCombos.length; fi++) {
    if (finalCombo.length >= 20) break;
    var c = allCombos[fi];
    if (fk.has(c.key)) continue;
    var sim = false;
    for (var si = 0; si < finalCombo.length; si++) {
      var sset = new Set(finalCombo[si].numbers);
      var o = 0;
      c.numbers.forEach(function (n) { if (sset.has(n)) o++; });
      if (o >= 3) { sim = true; break; }
    }
  ...
  return finalCombo.map(function (c) {
    var nsum = c.numbers.reduce(function (a, b) { return a + b; }, 0);
    var nsp = c.numbers[4] - c.numbers[0];
    var niv = [0, 0, 0]; c.numbers.forEach(function (n) { if (n <= 12) niv[0]++; else if (n <= 24) niv[1]++; else niv[2]++; });
    var nodd = c.numbers.filter(function (n) { return n % 2 === 1; }).length;
    return {
      numbers: c.numbers, key: c.key, score: c.score,
      sum: nsum, span: nsp, iv: niv.join(":"), odd: nodd,
      ratioKey: null, ratioText: c.strategy,
    };
  });
}

// ═══ v4.1 单组五行计划（覆盖优先策略）+ 补漏6 ═══
function buildV4SingleSamplePlan(frontCombos, backCombos, poolNumbers = null, candidateEntries = null, predictedTails = null, firstBallPreds = null, targetPatterns = null, sourceRow = null, allBalls = null) {
  const frontBase = frontCombos[0] || null;
  const backBase = backCombos[0] || null;
  if (!frontBase && !backBase) return [];
  ...
  if (targetPatterns && sourceRow && allBalls) {
    ...
    const mainSourceTails = [...new Set(srcNums.map(n => n % 10))];
    const prevRow = sourceRow - 1;
...
function selectCoverageOptimalCombos(allCombos, poolNumbers, n, tailRelationData = null) {
  ...
  for (let round = 1; round < n && selected.length < n; round++) {
    ...
    for (let i = 1; i < allCombos.length; i++) {
      ...
      // 综合评分：覆盖数 × 50 + 原始分数 + 尾号关系加分（覆盖优先但不忽视分数和尾号关系）
      const combined = newCoverage * 50 + c.score + tailRelationBonus;
...
sampleButton.addEventListener("click", () => {
  ...
  const variantPlan = isV4
    ? buildV4SingleSamplePlan(frontCombos, backCombos, poolNumbers, frontSample.candidateEntries, frontSample.predictedTails || null, firstBallPreds, targetPatterns, sourceStartRow, allBallsForPatterns)
...
// 策略驱动组合生成开关
// true: 使用策略驱动（区间匹配+尾号匹配+等距关系，不按评分排序）
// false: 使用传统评分排序（V5默认）
const USE_STRATEGY_DRIVEN = false;
...
if (__isNode) {
  ...
  console.log("script换策略.js — " + (USE_STRATEGY_DRIVEN ? "策略驱动组合生成（区间+尾号+等距关系）" : "V5 评分排序组合生成") + " + 多源融合");
  console.log("=".repeat(90));
  ...
  for (let sourceIdx = 1; sourceIdx <= totalDraws - PREDICT_INTERVAL - 1; sourceIdx++) {
    ...
    try {
      ...
      // 9. 生成完整方案（前区+后区）
      const variantPlan = [];
      for (let offset = 0; offset < Math.min(5, selectedFront.length); offset++) {
        const front = selectedFront[offset] || allCombos[0];
        const back = backCombos[offset % backCombos.length] || backCombos[0];
        variantPlan.push({
          key: `combo-${offset + 1}`,
          label: `第${offset + 1}行`,
          front, back,
          title: [`源${front.sourceRow}`, front?.ratioText ? `前区 ${front.ratioText}` : "", "后区自由"].filter(Boolean).join(" / "),
        });
      }
      
      // 10. 补漏6生成
      const firstBallPreds = mainSample.firstBallPredictions || null;
      ...
      // 🆕 尾号预测命中率统计
      const targetTails = [...new Set(targetNums.map(n => n % 10))]; // 目标号码的尾号（去重）
      ...
      results.push({
        ...
        top5Details, bl6Detail, poolHitNums,
      ...
      results.push({
        ...
        top5Details: null, bl6Detail: null, poolHitNums: [],
        error: err.message,
      });
    }
  }
  ...
  let cnt = 0;
  const csvRows = ['期数,目标号码,Top1命中,Top2命中,Top3命中,Top4命中,Top5命中,补漏6命中,Top5联合覆盖,Top5+补漏6联合覆盖,候选池覆盖'];
  ...
  results.forEach(r => {
    ...
    // Collect CSV data
    const t = r.top5Hits || [0,0,0,0,0];
    csvRows.push(`${cnt},"${r.targetNums.join(' ')}",${t[0]},${t[1]},${t[2]},${t[3]},${t[4]},${r.bl6Hits || 0},${r.top5UnionCoverage},${r.bl6UnionCoverage},${r.poolCoverage}`);
    ...
    console.log(`    候选池: [${r.poolNums.join(', ')}]`);
