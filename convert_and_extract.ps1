...
# Write as UTF-8
[System.IO.File]::WriteAllText('backtest_utf8.txt', $text, [System.Text.Encoding]::UTF8)
...
for ($i = 0; $i -lt $lines.Count; $i++) {
    ...
    # Target numbers  
    if ($line -match '\[(\d+),\s*(\d+),\s*(\d+),\s*(\d+),\s*(\d+)\]' -and $line -match '鐩') {
        $target = "$($Matches[1]) $($Matches[2]) $($Matches[3]) $($Matches[4]) $($Matches[5])"
    }
    
    # Top5 hit line
    if ($line -match 'Top1' -and $line -match 'Top5' -and $line -match '6:') {
        $hits = @()
    ...
    # Union coverage
    if ($line -match '鑱斿悎瑕嗙洊:\s+(\d+)' -and $line -notmatch '琛ユ紡6') {
        if ($results.Count -gt 0) {
            $results[-1].Top5Union = [int]$Matches[1]
        }
    }
    
    if ($line -match '琛ユ紡6\s+鑱斿悎瑕嗙洊:\s+(\d+)') {
        if ($results.Count -gt 0) {
    ...
    # Pool coverage
    if ($line -match '瑕嗙洊:\s+(\d+)\s*/\s*5') {
        if ($results.Count -gt 0 -and $line -match '鍊欓€夋睜') {
...
if ($results.Count -gt 0) {
    ...
    Write-Host "Saved per_period_detail.csv"
    ...
    Write-Host "Top5 联合覆盖平均: $([math]::Round($top5UnionAvg, 2))"
