...
function buildV4PlusTenTrendMap(sourceRow, sourceNumbers, boardBalls) {
  ...
  const weights = [0.4, 0.35, 0.25]; // 间隔9权重更高（回测最优配置）
...
if (__isNode) {
  ...
  const PREDICT_INTERVAL = 9; // 预测间隔10后的期数
  const SECOND_INTERVAL = 10;   // 第二间隔9（用于并集池合并）
  ...
  for (let sourceIdx = 1; sourceIdx <= totalDraws - PREDICT_INTERVAL - 1; sourceIdx++) {
    ...
    try {
      ...
      // 间隔10的源
      const auxSource1Idx = sourceIdx + 9;
      ...
      // 间隔12的源
      const auxSource2Idx = sourceIdx + 11;
