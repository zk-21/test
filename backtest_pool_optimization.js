/**
 * backtest_pool_optimization.js — 候选池构建逻辑优化测试
 * 
 * 基于最优参数（去重=3, 权重=0.75, 原版补盲），测试候选池优化：
 * 1. 候选池大小: poolSize (20, 22, 24, 26, 28)
 * 2. 区间保底策略: zoneGuarantee ([2,2,2], [3,3,3], [4,4,4])
 * 3. 尾号权重调整: tailWeight (0.7, 1.0, 1.3)
 * 
 * 运行：node backtest_pool_optimization.js
 */
...
function scoreSingleNumber(n, sourceNums, sourceTails, predictedTails, bridgeMap, arithMap,
  ...
  sourceSum, targetSum, historyMetrics, tailWeight) {
  let score = 0;
  let minOff = Infinity;
  ...
  const t = n % 10;
  const tw = tailWeight || 1.0;
  if (predictedTails && predictedTails.length > 0) {
    const topTails = new Set(predictedTails.slice(0, 5).map(tt => tt[0] || tt));
    if (topTails.has(t)) score += Math.round(V4_TAIL_SAME * tw);
    else if (predictedTails.some(tt => Math.abs(t - (tt[0] || tt)) === 1)) score += Math.round(V4_TAIL_NEIGHBOR * tw);
    else if (sourceTails.includes(t)) score += Math.round(V4_TAIL_WITHIN * tw);
  } else {
    if (sourceTails.includes(t)) score += Math.round(V4_TAIL_WITHIN * tw);
  }
...
// ======================== 15. 单期回测（参数化版本） ========================
function backtestSingle(sourceIdx, allDraws, dedupThreshold, diversityWeight, bulouStrategy, poolSize, zoneGuarantee, tailWeight) {
  ...
  for (const c of candidates) {
    if (finalCandidates.length >= poolSize) break;
    const z = getSampleIntervalIndex(c.number);
    if (z < 0) continue;
    if (zoneCount[z] < zoneGuarantee[z]) {
  ...
  const dimData = prepareDimData(finalCandidates, sourceNums, predictedTails);
  ...
  const poolSet = new Set(finalCandidates.slice(0, poolSize).map(c => c.number));
  const poolCoverage = targetNums.filter(n => poolSet.has(n)).length;

  // 使用参数化补漏6
  const bl6 = generate补漏6Optimized(combos, sourceNums, predictedTails, firstBallPreds, finalCandidates, bulouStrategy);
...
const paramSets = [
  // 基线
  { name: '基线(24球,[3,3,3])',  dedup: 3, divW: 0.75, bulou: 'original', pool: 24, zone: [3,3,3], tailW: 1.0 },
  // 池大小测试
  { name: '池=20',               dedup: 3, divW: 0.75, bulou: 'original', pool: 20, zone: [3,3,3], tailW: 1.0 },
  { name: '池=22',               dedup: 3, divW: 0.75, bulou: 'original', pool: 22, zone: [3,3,3], tailW: 1.0 },
  { name: '池=26',               dedup: 3, divW: 0.75, bulou: 'original', pool: 26, zone: [3,3,3], tailW: 1.0 },
  { name: '池=28',               dedup: 3, divW: 0.75, bulou: 'original', pool: 28, zone: [3,3,3], tailW: 1.0 },
  { name: '池=30',               dedup: 3, divW: 0.75, bulou: 'original', pool: 30, zone: [3,3,3], tailW: 1.0 },
  // 区间保底测试
  { name: '保底=[2,2,2]',        dedup: 3, divW: 0.75, bulou: 'original', pool: 24, zone: [2,2,2], tailW: 1.0 },
  { name: '保底=[4,4,4]',        dedup: 3, divW: 0.75, bulou: 'original', pool: 24, zone: [4,4,4], tailW: 1.0 },
  { name: '保底=[3,2,2]',        dedup: 3, divW: 0.75, bulou: 'original', pool: 24, zone: [3,2,2], tailW: 1.0 },
  // 尾号权重测试
  { name: '尾号×0.7',            dedup: 3, divW: 0.75, bulou: 'original', pool: 24, zone: [3,3,3], tailW: 0.7 },
  { name: '尾号×1.3',            dedup: 3, divW: 0.75, bulou: 'original', pool: 24, zone: [3,3,3], tailW: 1.3 },
  // 组合优化
  { name: '池=26+保底[2,2,2]',   dedup: 3, divW: 0.75, bulou: 'original', pool: 26, zone: [2,2,2], tailW: 1.0 },
  { name: '池=28+保底[2,2,2]',   dedup: 3, divW: 0.75, bulou: 'original', pool: 28, zone: [2,2,2], tailW: 1.0 },
  { name: '池=26+尾号×1.3',      dedup: 3, divW: 0.75, bulou: 'original', pool: 26, zone: [3,3,3], tailW: 1.3 },
  { name: '全优化A',              dedup: 3, divW: 0.75, bulou: 'original', pool: 28, zone: [2,2,2], tailW: 1.3 },
  { name: '全优化B',              dedup: 3, divW: 0.75, bulou: 'original', pool: 26, zone: [2,2,2], tailW: 1.3 },
];

// ======================== 17. 主运行 ========================
console.log('='.repeat(100));
console.log('候选池构建逻辑优化测试（基于最优参数：去重=3, 权重=0.75, 原版补盲）');
console.log('='.repeat(100));
...
for (const ps of paramSets) {
  ...
  for (let i = 0; i < DRAWS.length - 1; i++) {
    const r = backtestSingle(i, DRAWS, ps.dedup, ps.divW, ps.bulou, ps.pool, ps.zone, ps.tailW);
    if (!r) continue;
    stats.cnt++;
  ...
  allResults.push({ name: ps.name, dedup: ps.dedup, divW: ps.divW, bulou: ps.bulou, pool: ps.pool, zone: ps.zone, tailW: ps.tailW, stats });
}

// ======================== 18. 输出对比表格 ========================
console.log('');
console.log('='.repeat(140));
console.log('候选池优化对比结果');
console.log('='.repeat(140));
console.log('');
...
// 表头
const header = '参数方案'.padEnd(22) + '池大小'.padEnd(8) + '区间保底'.padEnd(12) + '尾号权重'.padEnd(10)
  + '命中率'.padEnd(16) + '总覆盖率'.padEnd(16) + '池覆盖率'.padEnd(10)
...
const basePool = baseline.sum_pool / (cnt * 5) * 100;
...
for (const r of allResults) {
  ...
  const pool = (s.sum_pool / (cnt * 5) * 100).toFixed(1);
  ...
  const zoneStr = `[${r.zone.join(',')}]`;

  console.log(
    r.name.padEnd(22) + String(r.pool).padEnd(8) + zoneStr.padEnd(12) + r.tailW.toFixed(1).padEnd(10)
    + rateStr.padEnd(16) + covStr.padEnd(16) + `${pool}%`.padEnd(10)
...
console.log(`  最高池覆盖: ${bestPool.name} (${(bestPool.stats.sum_pool / (cnt * 5) * 100).toFixed(1)}%)`);
...
console.log('池大小分析');
console.log('='.repeat(80));
const poolResults = allResults.filter(r => r.name.startsWith('池='));
if (poolResults.length > 0) {
  console.log('  池大小 | 命中率 | 总覆盖率 | 池覆盖率');
  console.log('  ───────┼────────┼──────────┼─────────');
  poolResults.forEach(r => {
    const rate = (r.stats.sum_top5 / (cnt * 5) * 100).toFixed(1);
    const cov = (r.stats.sum_total / (cnt * 5) * 100).toFixed(1);
    const pool = (r.stats.sum_pool / (cnt * 5) * 100).toFixed(1);
    console.log(`  ${String(r.pool).padStart(2)}球   | ${rate.padStart(5)}% | ${cov.padStart(7)}% | ${pool.padStart(6)}%`);
  });
}
