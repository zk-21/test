/**
 * backtest_v5_nopair_random.js — 对比：A) 6维配对+排序  vs  B) 6维独立+随机加权
 * 
 * A) 原版配对+评分排序：tail×0.55+offset×0.45 →3注, hot×0.50+freq×0.50 →3注, bridge×0.50+arith×0.50 →2注
 * B) 独立+随机加权：tail→2注, offset→2注, hot→1注, freq→1注, bridge→1注, arith→1注（加权随机抽取）
 * 
 * 运行：node backtest_v5_nopair_random.js
 */
...
function genCombosRandom(pool, count, maxPool, weightPower = 0.5) {
  ...
  for (let attempt = 0; attempt < maxAttempts && combos.length < count * 10; attempt++) {
    const nums = weightedRandomPick(poolRange, 5, weightPower);
    if (!isValidCombo(nums)) continue;
    const key = nums.join('-');
    if (seen.has(key)) continue;
    seen.add(key);
...
// ======================== 12. 方案A：6维配对 + 评分排序 ========================
function buildCombosPaired(dimData, sourceNums, predictedTails) {
  ...
  genCombosFromPool(pair1, 3, 13).forEach(c => { if (!usedKeys.has(c.key)) { allCombos.push(c); usedKeys.add(c.key); } });
  genCombosFromPool(pair2, 3, 13).forEach(c => { if (!usedKeys.has(c.key)) { allCombos.push(c); usedKeys.add(c.key); } });
  genCombosFromPool(pair3, 2, 10).forEach(c => { if (!usedKeys.has(c.key)) { allCombos.push(c); usedKeys.add(c.key); } });

  if (allCombos.length < 8) {
    const suppPool = new Map();
    [pair1, pair2, pair3].forEach(dim => {
...
// ======================== 13. 方案B：6维独立 + 加权随机 ========================
function buildCombosIndependentRandom(dimData, sourceNums, predictedTails) {
  const { nTail, nOff, nHot, nFreq, nBr, nAr } = dimData;
  ...
  let allCombos = []; const usedKeys = new Set();
  dims.forEach(d => {
    genCombosRandom(d.arr, d.count, d.maxPool, 0.5).forEach(c => {
      if (!usedKeys.has(c.key)) { allCombos.push(c); usedKeys.add(c.key); }
    });
  });
  ...
  if (allCombos.length < 8) {
    ...
    const suppArr = [...suppPool.values()].sort((a, b) => b.score - a.score);
    genCombosRandom(suppArr, 8 - allCombos.length, 13, 0.5).forEach(c => {
      if (!usedKeys.has(c.key)) { allCombos.push(c); usedKeys.add(c.key); }
    });
  }
...
function backtestBoth(sourceIdx, allDraws) {
  ...
  // A: 配对+评分排序
  const combosA = buildCombosPaired(dimData, sourceNums, predictedTails);
  // B: 独立+加权随机
  const combosB = buildCombosIndependentRandom(dimData, sourceNums, predictedTails);
...
console.log('对比测试：A) 6维配对+评分排序  vs  B) 6维独立+加权随机');
console.log('='.repeat(100));
...
// ======================== 18. 输出 ========================
console.log('逐期对比:');
console.log('源期号    目标期号   Top5最高命中(A/B)   联合覆盖(A/B)   补漏6(A/B)   总覆盖(A/B)   候选池覆盖');
console.log('-'.repeat(100));

let sumA_top5 = 0, sumA_union = 0, sumA_bl6 = 0, sumA_total = 0, sumA_pool = 0;
let sumB_top5 = 0, sumB_union = 0, sumB_bl6 = 0, sumB_total = 0, sumB_pool = 0;
let winA_top5 = 0, winB_top5 = 0, tie_top5 = 0;
let winA_union = 0, winB_union = 0, tie_union = 0;
let winA_total = 0, winB_total = 0, tie_total = 0;
let cnt = 0;

results.forEach(r => {
  cnt++;
  sumA_top5 += r.A.top5MaxHit; sumB_top5 += r.B.top5MaxHit;
  sumA_union += r.A.top5UnionCoverage; sumB_union += r.B.top5UnionCoverage;
  sumA_bl6 += r.A.bl6Hits; sumB_bl6 += r.B.bl6Hits;
  sumA_total += r.A.bl6UnionCoverage; sumB_total += r.B.bl6UnionCoverage;
  sumA_pool += r.A.poolCoverage; sumB_pool += r.B.poolCoverage;

  if (r.A.top5MaxHit > r.B.top5MaxHit) winA_top5++;
  else if (r.B.top5MaxHit > r.A.top5MaxHit) winB_top5++;
  else tie_top5++;

  if (r.A.top5UnionCoverage > r.B.top5UnionCoverage) winA_union++;
  else if (r.B.top5UnionCoverage > r.A.top5UnionCoverage) winB_union++;
  else tie_union++;

  if (r.A.bl6UnionCoverage > r.B.bl6UnionCoverage) winA_total++;
  else if (r.B.bl6UnionCoverage > r.A.bl6UnionCoverage) winB_total++;
  else tie_total++;

  const line = [
    r.sourceIssue.padEnd(10),
    r.targetIssue.padEnd(10),
    `${r.A.top5MaxHit}/${r.B.top5MaxHit}`.padEnd(20),
    `${r.A.top5UnionCoverage}/${r.B.top5UnionCoverage}`.padEnd(17),
    `${r.A.bl6Hits}/${r.B.bl6Hits}`.padEnd(14),
    `${r.A.bl6UnionCoverage}/${r.B.bl6UnionCoverage}`.padEnd(14),
    `${r.A.poolCoverage}`.padEnd(13),
    r.sourceNums.join(','),
  ].join(' | ');
  console.log(line);
});
...
console.log('');
console.log('='.repeat(100));
console.log('汇总对比：A) 配对+评分排序  vs  B) 6维独立+加权随机');
console.log('='.repeat(100));
console.log('');
console.log('指标'.padEnd(25) + 'A)配对+排序'.padEnd(16) + 'B)独立+随机'.padEnd(16) + '差异'.padEnd(12) + '胜者');
console.log('-'.repeat(69));
console.log('Top5平均最高命中'.padEnd(25) + `${(sumA_top5/cnt).toFixed(2)}/5`.padEnd(16) + `${(sumB_top5/cnt).toFixed(2)}/5`.padEnd(16) + `${(sumB_top5-sumA_top5>=0?'+':'')+(sumB_top5/cnt-sumA_top5/cnt).toFixed(2)}`.padEnd(12) + (sumB_top5>=sumA_top5?'B':'A'));
console.log('Top5联合覆盖率'.padEnd(25) + `${(sumA_union/cnt).toFixed(2)}/5`.padEnd(16) + `${(sumB_union/cnt).toFixed(2)}/5`.padEnd(16) + `${(sumB_union-sumA_union>=0?'+':'')+(sumB_union/cnt-sumA_union/cnt).toFixed(2)}`.padEnd(12) + (sumB_union>=sumA_union?'B':'A'));
console.log('补漏6平均命中'.padEnd(25) + `${(sumA_bl6/cnt).toFixed(2)}/5`.padEnd(16) + `${(sumB_bl6/cnt).toFixed(2)}/5`.padEnd(16) + `${(sumB_bl6-sumA_bl6>=0?'+':'')+(sumB_bl6/cnt-sumA_bl6/cnt).toFixed(2)}`.padEnd(12) + (sumB_bl6>=sumA_bl6?'B':'A'));
console.log('补漏6后总覆盖率'.padEnd(25) + `${(sumA_total/cnt).toFixed(2)}/5`.padEnd(16) + `${(sumB_total/cnt).toFixed(2)}/5`.padEnd(16) + `${(sumB_total-sumA_total>=0?'+':'')+(sumB_total/cnt-sumA_total/cnt).toFixed(2)}`.padEnd(12) + (sumB_total>=sumA_total?'B':'A'));
console.log('候选池覆盖'.padEnd(25) + `${(sumA_pool/cnt).toFixed(2)}/5`.padEnd(16) + `${(sumB_pool/cnt).toFixed(2)}/5`.padEnd(16) + '相同'.padEnd(12) + '—');

console.log('');
console.log('Top5 最高命中 逐期胜负:');
console.log(`  A胜: ${winA_top5}期  B胜: ${winB_top5}期  平: ${tie_top5}期`);

console.log('');
console.log('Top5 联合覆盖 逐期胜负:');
console.log(`  A胜: ${winA_union}期  B胜: ${winB_union}期  平: ${tie_union}期`);

console.log('');
console.log('总覆盖(含补漏6) 逐期胜负:');
console.log(`  A胜: ${winA_total}期  B胜: ${winB_total}期  平: ${tie_total}期`);

// 命中率
const rateA_top5 = (sumA_top5 / (cnt * 5) * 100).toFixed(1);
const rateB_top5 = (sumB_top5 / (cnt * 5) * 100).toFixed(1);
const rateA_total = (sumA_total / (cnt * 5) * 100).toFixed(1);
const rateB_total = (sumB_total / (cnt * 5) * 100).toFixed(1);
console.log('');
console.log(`  命中率: A=${rateA_top5}%  B=${rateB_top5}%`);
console.log(`  总覆盖率: A=${rateA_total}%  B=${rateB_total}%`);

// 命中分布
const distA = {}, distB = {};
results.forEach(r => {
  distA[r.A.top5MaxHit] = (distA[r.A.top5MaxHit] || 0) + 1;
  distB[r.B.top5MaxHit] = (distB[r.B.top5MaxHit] || 0) + 1;
});

console.log('');
console.log('  Top5 最高命中分布:');
console.log('  命中数 | A)配对+排序 | B)独立+随机 | 差异');
console.log('  -------|------------|------------|------');
for (let h = 5; h >= 0; h--) {
  const a = distA[h] || 0, b = distB[h] || 0;
  if (a > 0 || b > 0) {
    const diff = b - a;
    const diffStr = diff >= 0 ? `+${diff}` : `${diff}`;
    console.log(`  ${h}个    | ${String(a).padEnd(10)} | ${String(b).padEnd(10)} | ${diffStr}`);
  }
}
