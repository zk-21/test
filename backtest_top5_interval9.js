/**
 * 全量回测：每一期Top5号码命中测试（间隔9策略）
 * 数据：optimized_picker.js 内嵌数据
 */

const fs = require('fs');
const path = require('path');

// 加载optimized_picker.js，但去掉CLI处理部分
let code = fs.readFileSync(path.join(__dirname, 'optimized_picker.js'), 'utf-8');

// 移除所有 CLI 处理代码（从 const args = process.argv 开始到最后）
const cliStart = code.indexOf('\nconst args = process.argv.slice');
if (cliStart > 0) {
  code = code.substring(0, cliStart);
}
