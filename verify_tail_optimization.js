...
// 优化后的尾号模式评分
function scoreTailPatterns(sorted) {
  const tails = [...new Set(sorted.map(n => n % 10))].sort((a,b) => a-b);
  let score = 0;
  ...
  for (let d = 2; d <= 4; d++) {
    for (let start = 0; start <= 9 - d * 2; start++) {
      let count = 0;
      for (let v = start; v <= 9; v += d) { if (tails.includes(v)) count++; else break; }
      if (count >= 4) score += 30;
...
// 基线尾号模式评分（优化前）
function scoreTailPatterns_old(sorted) {
  const tails = [...new Set(sorted.map(n => n % 10))].sort((a,b) => a-b);
  let score = 0;
  let longestConsec = 1, currentConsec = 1;
  for (let i = 1; i < tails.length; i++) {
  ...
  if (longestConsec >= 3) score += 30;
  else if (longestConsec >= 2) score += 15;
  for (let d = 2; d <= 4; d++) {
    for (let start = 0; start <= 9 - d * 2; start++) {
      let count = 0;
      for (let v = start; v <= 9; v += d) { if (tails.includes(v)) count++; else break; }
...
// 组合评分
function scoreCombo(sorted, baseScores, tailFn, tailWeight) {
  ...
  const tailScore = tailFn(sorted);
  comboBonus += tailScore * tailWeight;

  return baseScore + comboBonus;
}

// 回测
function backtest(tailFn, tailWeight, label) {
  const startIdx = 15;
        ...
        for (let c = b+1; c < top25.length && combos.length < maxCombos; c++) {
          for (let d = c+1; d < top25.length && combos.length < maxCombos; d++) {
            for (let e = d+1; e < top25.length && combos.length < maxCombos; e++) {
              ...
              const score = scoreCombo(combo, trendScores, tailFn, tailWeight);
...
console.log('═══════════════════════════════════════════════════════════');
console.log('  尾号优化验证：优化前 vs 优化后');
console.log('═══════════════════════════════════════════════════════════');
console.log(`  数据: ${ALL_DRAWS.length}期`);
console.log('');

const r_old = backtest(scoreTailPatterns_old, 0.4, '优化前(基线)');
const r_new = backtest(scoreTailPatterns, 0.6, '优化后(高尾号权重)');

console.log('  ┌────────────────┬────────┬────────┬────────┬────────┬────────┐');
console.log('  │ 配置            │ Top5   │ Top5≥3 │ Top5零 │ Top10  │ Top10≥3│');
console.log('  ├────────────────┼────────┼────────┼────────┼────────┼────────┤');
console.log(`  │ ${r_old.label.padEnd(14)} │ ${r_old.top5Rate.padStart(5)}% │ ${r_old.top5ge3.padStart(5)}% │ ${String(r_old.top5zero).padStart(4)}   │ ${r_old.top10Rate.padStart(5)}% │ ${r_old.top10ge3.padStart(5)}% │`);
console.log(`  │ ${r_new.label.padEnd(14)} │ ${r_new.top5Rate.padStart(5)}% │ ${r_new.top5ge3.padStart(5)}% │ ${String(r_new.top5zero).padStart(4)}   │ ${r_new.top10Rate.padStart(5)}% │ ${r_new.top10ge3.padStart(5)}% │`);
console.log('  └────────────────┴────────┴────────┴────────┴────────┴────────┘');

const diff5 = (parseFloat(r_new.top5Rate) - parseFloat(r_old.top5Rate)).toFixed(1);
const diffZero = r_new.top5zero - r_old.top5zero;
console.log(`\n  📊 变化: Top5命中率 ${diff5 >= 0 ? '+' : ''}${diff5}% | 零球数 ${diffZero >= 0 ? '+' : ''}${diffZero}`);

if (parseFloat(r_new.top5Rate) > parseFloat(r_old.top5Rate)) {
  console.log('  ✅ 优化有效！');
} else if (parseFloat(r_new.top5Rate) === parseFloat(r_old.top5Rate)) {
  console.log('  ⚠️ 效果持平');
} else {
  console.log('  ❌ 优化无效');
}
