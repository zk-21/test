...
console.log('═══════════════════════════════════════════════════════════');
console.log('  结构约束参数分析与优化');
console.log('═══════════════════════════════════════════════════════════');
console.log(`  数据: ${ALL_DRAWS.length}期`);
console.log('');

// ===== 1. 分析历史数据的结构分布 =====
const sums = [], spans = [], odds = [], ivs = [];
ALL_DRAWS.forEach(d => {
  const nums = [...d.front].sort((a,b)=>a-b);
  sums.push(nums.reduce((a,b)=>a+b,0));
  spans.push(nums[nums.length-1] - nums[0]);
  odds.push(nums.filter(n=>n%2===1).length);
  const iv = [0,0,0];
  nums.forEach(n => iv[Math.floor((n-1)/5)]++);
  ivs.push(iv);
});
...
function stats(arr, label) {
  ...
  console.log(`  ${label}:`);
  console.log(`    均值=${avg.toFixed(1)} | 中位数=${median} | 最小=${min} | 最大=${max}`);
  console.log(`    P10=${p10} | P25=${p25} | P75=${p75} | P90=${p90}`);
  console.log(`    当前设置: target=${label==='和值'?90:label==='跨度'?24:'N/A'}, tolerance=${label==='和值'?25:label==='跨度'?10:'N/A'}`);
  console.log(`    当前范围: [${label==='和值'?65:label==='跨度'?14:'N/A'}, ${label==='和值'?115:label==='跨度'?34:'N/A'}]`);
  console.log(`    覆盖率: ${label==='和值'?
    ((sums.filter(s=>s>=65&&s<=115).length/sums.length*100).toFixed(1))+'%':
    label==='跨度'?
    ((spans.filter(s=>s>=14&&s<=34).length/spans.length*100).toFixed(1))+'%':'N/A'}`);
  console.log('');
}
...
// 奇偶分布
const oddDist = {};
odds.forEach(o => oddDist[o] = (oddDist[o]||0)+1);
console.log('  奇数个数分布:');
for (let k = 0; k <= 5; k++) {
  const cnt = oddDist[k] || 0;
  console.log(`    ${k}个奇数: ${cnt}次 (${(cnt/odds.length*100).toFixed(1)}%)`);
}
console.log('');
...
ivs.forEach(iv => {
  const key = iv.join(':');
  ivDist[key] = (ivDist[key]||0)+1;
});
console.log('  区间比分布(Top10):');
const sortedIv = Object.entries(ivDist).sort((a,b)=>b[1]-a[1]).slice(0,10);
sortedIv.forEach(([k,v]) => console.log(`    ${k}: ${v}次 (${(v/ivs.length*100).toFixed(1)}%)`));
console.log('');
...
// 组合评分（带结构约束过滤）
function scoreCombo(sorted, baseScores, cfg) {
  const s = sorted.reduce((a,b) => a+b, 0);
  const sp = sorted[sorted.length-1] - sorted[0];
  const odd = sorted.filter(n => n%2===1).length;
  if (odd === 0 || odd === 5) return -9999;
  if (sp < cfg.minSpan || sp > cfg.maxSpan) return -9999;
  if (s < cfg.minSum || s > cfg.maxSum) return -9999;
  ...
  let comboBonus = 0;
  if (s >= cfg.sumSweetMin && s <= cfg.sumSweetMax) comboBonus += 6;
  if (sp >= cfg.spanSweetMin && sp <= cfg.spanSweetMax) comboBonus += 5;
...
function backtest(cfg, label) {
        ...
        for (let c = b+1; c < top25.length && combos.length < maxCombos; c++) {
          for (let d = c+1; d < top25.length && combos.length < maxCombos; d++) {
            for (let e = d+1; e < top25.length && combos.length < maxCombos; e++) {
              ...
              combosTotal++;
              const score = scoreCombo(combo, trendScores, cfg);
              if (score > -9999) {
  ...
  return {
    ...
    passRate: (combosPassed / combosTotal * 100).toFixed(1),
  };
}
...
const configs = [
  {
    label: '基线(sum90±25,span24±10)',
    minSum: 65, maxSum: 115, minSpan: 14, maxSpan: 34,
    sumSweetMin: 60, sumSweetMax: 120, spanSweetMin: 15, spanSweetMax: 30
  },
  {
    label: '放宽和值(sum85±30)',
    minSum: 55, maxSum: 115, minSpan: 14, maxSpan: 34,
    sumSweetMin: 55, sumSweetMax: 115, spanSweetMin: 15, spanSweetMax: 30
  },
  {
    label: '收紧和值(sum90±15)',
    minSum: 75, maxSum: 105, minSpan: 14, maxSpan: 34,
    sumSweetMin: 75, sumSweetMax: 105, spanSweetMin: 15, spanSweetMax: 30
  },
  {
    label: '放宽跨度(span24±15)',
    minSum: 65, maxSum: 115, minSpan: 9, maxSpan: 34,
    sumSweetMin: 60, sumSweetMax: 120, spanSweetMin: 10, spanSweetMax: 34
  },
  {
    label: '收紧跨度(span24±5)',
    minSum: 65, maxSum: 115, minSpan: 19, maxSpan: 29,
    sumSweetMin: 60, sumSweetMax: 120, spanSweetMin: 19, spanSweetMax: 29
  },
  {
    label: '数据驱动(sum88±22,span22±8)',
    minSum: 66, maxSum: 110, minSpan: 14, maxSpan: 30,
    sumSweetMin: 66, sumSweetMax: 110, spanSweetMin: 14, spanSweetMax: 30
  },
  {
    label: '宽松(无和值跨度约束)',
    minSum: 15, maxSum: 160, minSpan: 3, maxSpan: 34,
    sumSweetMin: 60, sumSweetMax: 120, spanSweetMin: 15, spanSweetMax: 30
  },
  {
    label: '仅和值约束(sum88±20)',
    minSum: 68, maxSum: 108, minSpan: 3, maxSpan: 34,
    sumSweetMin: 68, sumSweetMax: 108, spanSweetMin: 15, spanSweetMax: 30
  },
  {
    label: '仅跨度约束(span22±8)',
    minSum: 15, maxSum: 160, minSpan: 14, maxSpan: 30,
    sumSweetMin: 60, sumSweetMax: 120, spanSweetMin: 14, spanSweetMax: 30
  },
  {
    label: '极宽和值(50~130)',
    minSum: 50, maxSum: 130, minSpan: 14, maxSpan: 34,
    sumSweetMin: 50, sumSweetMax: 130, spanSweetMin: 15, spanSweetMax: 30
  },
];

console.log('═══════════════════════════════════════════════════════════');
console.log('  结构约束参数回测对比');
console.log('═══════════════════════════════════════════════════════════');
console.log('');

const results = [];
for (const cfg of configs) {
  results.push(backtest(cfg, cfg.label));
}

results.sort((a, b) => parseFloat(b.top5Rate) - parseFloat(a.top5Rate));

console.log('  ┌──────────────────────────────────┬────────┬────────┬────────┬────────┬────────┬────────┐');
console.log('  │ 配置                              │ Top5   │ Top5≥3 │ Top5零 │ Top10  │ Top10≥3│ 通过率 │');
console.log('  ├──────────────────────────────────┼────────┼────────┼────────┼────────┼────────┼────────┤');
for (const r of results) {
  const name = r.label.padEnd(32);
  console.log(`  │ ${name} │ ${r.top5Rate.padStart(5)}% │ ${r.top5ge3.padStart(5)}% │ ${String(r.top5zero).padStart(4)}   │ ${r.top10Rate.padStart(5)}% │ ${r.top10ge3.padStart(5)}% │ ${r.passRate.padStart(5)}% │`);
}
console.log('  └──────────────────────────────────┴────────┴────────┴────────┴────────┴────────┴────────┘');
