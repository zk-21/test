...
function buildV4PlusTenTrendMap(sourceRow, sourceNumbers, boardBalls) {
  ...
  boardBalls.filter(b => b.zone === "front" && ballHasColor(b, sampleRedColor)).forEach(b => {
...
const V4_TAIL_SAME = 35, V4_TAIL_NEIGHBOR = 20, V4_TAIL_NEIGHBOR2 = 10, V4_TAIL_WITHIN = 8;
...
function predictTargetOddCount(sourceRow, boardBalls) {
  ...
  // 优化：预分组避免重复filter
  const _br = new Map();
  boardBalls.filter(b => b.zone === "front").forEach(b => {
    if (!(_br.has(b.row))) _br.set(b.row, []);
    _br.get(b.row).push(b);
  });
...
// ═══ v5: 分区间独立预测（拆分区间比为一区/二区/三区独立分析）═══
function predictZoneDirections(sourceRow, boardBalls) {
  // 预分组
  const ballsByRow = new Map();
  boardBalls.filter(b => b.zone === "front").forEach(b => {
    if (!ballsByRow.has(b.row)) ballsByRow.set(b.row, []);
    ballsByRow.get(b.row).push(b);
  });
  ...
  const seenRows = new Set();
  for (let r = 1; r <= sourceRow; r++) {
    const nums = [...new Set((ballsByRow.get(r) || []).filter(b => ballHasColor(b, sampleRedColor)).map(b => b.number))].sort((a, b) => a - b);
    if (nums.length === 5 && !seenRows.has(r)) {
      seenRows.add(r);
      draws.push(nums);
    }
  ...
  const sourceIv = intervalRatio(draws[draws.length - 1]);
  const lookback = Math.min(15, draws.length - 1);
  ...
  for (let z = 0; z < 3; z++) {
    // 1. 最近N期该区间数量序列
    const recent = [];
    for (let i = draws.length - lookback; i < draws.length; i++) {
      recent.push(intervalRatio(draws[i])[z]);
    }
    const current = sourceIv[z];
    
    // 2. 历史均值
    let totalSum = 0, totalCount = 0;
    for (let i = 0; i < draws.length; i++) {
      totalSum += intervalRatio(draws[i])[z];
      totalCount++;
    }
    ...
    for (let i = 1; i < recent5.length; i++) {
      trendSum += (recent5[i] - recent5[i - 1]);
    }
    const trend = trendSum / (recent5.length - 1); // 平均每期变化
    
    // 4. 转移概率：当前值→下一期值的分布
    const transitionMap = new Map();
    for (let i = 0; i < draws.length - 1; i++) {
      const cv = intervalRatio(draws[i])[z];
      const nv = intervalRatio(draws[i + 1])[z];
      const key = cv + "->" + nv;
    ...
    for (const [key, count] of transitionMap) {
      if (key.startsWith(current + "->")) {
        const nextVal = parseInt(key.split("->")[1]);
        nextProbs.set(nextVal, (nextProbs.get(nextVal) || 0) + count);
        transTotal += count;
      }
    }
    
    // 5. 综合评分：预测下期该区间数量
    let expectedValue = current; // 默认不变
    ...
    if (transTotal > 0) {
      let transExpected = 0;
      for (const [val, count] of nextProbs) {
        transExpected += val * (count / transTotal);
      }
      expectedValue = transExpected;
    }
    
    // 5b. 趋势修正（最近5期方向）
    expectedValue += trend * 0.5;
    
    // 5c. 极值回归（强信号）
    if (current === 0) {
      expectedValue = Math.max(expectedValue, 1.5); // 强制回归
      strength += 5;
    } else if (current >= 4) {
      expectedValue = Math.min(expectedValue, 2.0); // 强制回归
      strength += 5;
    }
    
    // 5d. 均值回归
    if (current < mean - 0.5) {
      expectedValue += (mean - current) * 0.3;
      strength += 2;
    } else if (current > mean + 0.5) {
      expectedValue -= (current - mean) * 0.3;
      strength += 2;
    }
    
    // 确定方向
    const diff = expectedValue - current;
    if (diff > 0.3) direction = "up";
    else if (diff < -0.3) direction = "down";
    
    // 强度：|diff|越大越强
    strength += Math.min(5, Math.abs(diff) * 3);
...
function analyzeTailComboTransitions(sourceRow, lookback, allBalls) {
  ...
  // 预先按行分组
  const ballsByRow = new Map();
  balls.filter(b => b.zone === "front").forEach(b => {
    if (!ballsByRow.has(b.row)) ballsByRow.set(b.row, []);
    ballsByRow.get(b.row).push(b.number);
  });
...
// ═══ 分析目标号码尾号模式（关联性、分布规律、时序规律）═══
function analyzeTargetTailPatterns(sourceRow, allBalls, lookback = 100) {
  const tailPairFreq = new Map();
  const tailTripletFreq = new Map();
  const consecutiveTripletFreq = new Map();
  const arithmeticTripletFreq = new Map();
  ...
  // 分布规律统计
  const zoneDist = [0, 0, 0]; // 一区、二区、三区
  const oddEvenDist = [0, 0]; // 奇数、偶数
  const bigSmallDist = [0, 0]; // 大数(>=5)、小数(<5)
  
  // 时序规律统计
  const tailChangeFreq = new Map(); // 尾号变化模式
  const tailTrendFreq = new Map(); // 尾号趋势（上升、下降、持平）
  ...
  for (let r = start; r < sourceRow; r++) {
    ...
    for (let i = 0; i < drawTails.length; i++) {
      for (let j = i + 1; j < drawTails.length; j++) {
        for (let k = j + 1; k < drawTails.length; k++) {
          ...
          if (isConsecutive) {
            consecutiveTripletFreq.set(triplet, (consecutiveTripletFreq.get(triplet) || 0) + 1);
          }
          ...
          for (let step = 1; step <= 4; step++) {
            ...
            if (isArithmetic) {
              arithmeticTripletFreq.set(triplet, (arithmeticTripletFreq.get(triplet) || 0) + 1);
            }
          }
        }
      }
    ...
    nums.forEach(n => {
      if (n <= 12) zoneDist[0]++;
      else if (n <= 24) zoneDist[1]++;
      else zoneDist[2]++;
    });
    
    // 奇偶分布
    nums.forEach(n => {
      if (n % 2 === 1) oddEvenDist[0]++;
      else oddEvenDist[1]++;
    });
    
    // 大小分布（尾号>=5为大，<5为小）
    drawTails.forEach(t => {
      if (t >= 5) bigSmallDist[0]++;
      else bigSmallDist[1]++;
    });
    ...
    // 记录尾号变化模式（与前一期比较）
    if (r > start) {
      const prevNums = [...new Set(ballsByRow.get(r - 1) || [])].sort((a, b) => a - b);
      if (prevNums.length === 5) {
        const prevTails = [...new Set(prevNums.map((n) => n % 10))].sort((a, b) => a - b);
        
        // 计算尾号变化（相同、相邻、远离）
        let sameCount = 0, neighborCount = 0, farCount = 0;
        drawTails.forEach(t => {
          if (prevTails.includes(t)) sameCount++;
          else if (prevTails.some(pt => Math.abs(pt - t) === 1 || Math.abs(pt - t) === 9)) neighborCount++;
          else farCount++;
        });
        
        const changeKey = `same${sameCount}_neighbor${neighborCount}_far${farCount}`;
        tailChangeFreq.set(changeKey, (tailChangeFreq.get(changeKey) || 0) + 1);
        
        // 计算尾号趋势（上升、下降、持平）
        const drawSum = drawTails.reduce((a, b) => a + b, 0);
        const prevSum = prevTails.reduce((a, b) => a + b, 0);
        const trend = drawSum > prevSum ? 'up' : drawSum < prevSum ? 'down' : 'flat';
        tailTrendFreq.set(trend, (tailTrendFreq.get(trend) || 0) + 1);
      }
    }
  }
  
  // 计算分布规律比例
  const totalZone = zoneDist.reduce((a, b) => a + b, 0) || 1;
  const totalOddEven = oddEvenDist.reduce((a, b) => a + b, 0) || 1;
  const totalBigSmall = bigSmallDist.reduce((a, b) => a + b, 0) || 1;
  
  return {
    // 关联性模式
    tailPairFreq,
    tailTripletFreq,
    consecutiveTripletFreq,
    arithmeticTripletFreq,
    
    // 分布规律
    zoneDistribution: zoneDist.map(v => v / totalZone),
    oddEvenDistribution: oddEvenDist.map(v => v / totalOddEven),
    bigSmallDistribution: bigSmallDist.map(v => v / totalBigSmall),
    ...
    // 统计数据
    totalSamples: Math.max(1, sourceRow - start - 1),
  };
}
...
function getBridgeTailScores(tails) {
  ...
  for (let i = 0; i < sorted.length; i++) {
    for (let j = i + 1; j < sorted.length; j++) {
      ...
      // 计算两个方向的距离
      const dist1 = (t2 - t1 + 10) % 10; // 顺时针距离
      const dist2 = (t1 - t2 + 10) % 10; // 逆时针距离
      ...
      if (gap >= 2 && gap <= 4) {
        // 中间尾号数量 = gap - 1
        for (let k = 1; k < gap; k++) {
          const midTail = (start + step * k) % 10;
          // 根据间隙大小加分
          let bonus = 0;
          if (gap === 2) bonus = 3;      // 中间1个尾号
          else if (gap === 3) bonus = 2; // 中间2个尾号
          else if (gap === 4) bonus = 1; // 中间3个尾号
          
          bridgeScores.set(midTail, bridgeScores.get(midTail) + bonus);
        }
      }
    }
  }
...
function getCrossSetBridgeTailScores(tails1, tails2) {
  ...
  // 对于tails1中的每个尾号和tails2中的每个尾号
  for (const t1 of set1) {
    for (const t2 of set2) {
      if (t1 === t2) continue; // 跳过相同的尾号
      ...
      if (gap >= 2 && gap <= 4) {
        ...
        for (let k = 1; k < gap; k++) {
          ...
          // 根据间隙大小加分
          let bonus = 0;
          if (gap === 2) bonus = 4;      // 中间1个尾号，较高权重
          else if (gap === 3) bonus = 3; // 中间2个尾号
          else if (gap === 4) bonus = 2; // 中间3个尾号
...
const ABLATION_CONFIG = {
  disable_sameOrNeighbor: false,    // 启用相同或相邻尾号（+0.7pp覆盖率）
  disable_crossRowTail: true,       // 禁用跨行尾号（消融实验发现几乎无效）
  disable_intraGroupPattern: false, // 禁用组内模式
  disable_arithmeticNumber: false,  // 禁用完整号码等差
  disable_crossRowArithmetic: false,// 禁用跨行等差
  disable_bridgeTails: false,       // 禁用桥接规则
  disable_overlap1: false,          // 禁用重叠尾号
  disable_arith1: false,            // 禁用等差延伸
  disable_comboTransfer: false,     // 保留尾号组合转移模式（有正面贡献）
};

function predictLikelyTailsV4Enhanced(sourceTails, transData, refRows, sourceRow, allBalls = null) {
  // 🆕 第二次优化：基于信号分析的权重调整
  // 核心变化：降低globalFreq/sameOrNeighbor/intraGroupPattern/bridgeTails，提升tailArithmeticExtension(15)
  // 移除多样性强制机制（奇偶平衡while循环），让分数自然排序
  const weights = {
    overlap1: ABLATION_CONFIG.disable_overlap1 ? 0 : 4,      // 重叠尾号
    sameOrNeighbor: ABLATION_CONFIG.disable_sameOrNeighbor ? 0 : 15, // 相同或相邻尾号（基线值）
    arith1: ABLATION_CONFIG.disable_arith1 ? 0 : 6,        // 保持：等差模式
    overlap10: 2,     // 保持：远期参考影响较小
    arith10: 4,       // 保持：远期等差
    overlapBonus: 2,  // 保持：双重保留更可靠
    globalFreq: 15,   // 全局高频尾号（基线值）
    comboTransfer: ABLATION_CONFIG.disable_comboTransfer ? 0 : 2, // 保持：完全匹配率0%
    crossRowTail: ABLATION_CONFIG.disable_crossRowTail ? 0 : 10,   // 保持：跨行尾号
    intraGroupPattern: ABLATION_CONFIG.disable_intraGroupPattern ? 0 : 12, // 组内尾号规则（基线值）
    arithmeticNumber: ABLATION_CONFIG.disable_arithmeticNumber ? 0 : 8, // 保持
    crossRowArithmetic: ABLATION_CONFIG.disable_crossRowArithmetic ? 0 : 12, // 保持
    bridgeTails: ABLATION_CONFIG.disable_bridgeTails ? 0 : 8,         // 桥接尾号
    tailArithmeticExtension: ABLATION_CONFIG.disable_arith1 ? 0 : 12, // 等差延伸邻号（93.6%命中率）
    tailClustering: 8,       // 保持
    tailTransition: 10,      // 保持
    tailPeriodicity: 8,      // 保持
    tailInterval: 8,         // 保持
    tailOddEvenSize: 10,     // 保持：奇偶大小组合
  };
  ...
  if (refRows && refRows.length > 0) {
    ...
    if (ref1) {
      ...
      ref1.tailSet.forEach(t => {
        scores.set(t, scores.get(t) + weights.overlap10); // 使用overlap10权重
      });
  ...
  if (allBalls && sourceRow > 0) {
    const comboTransData = analyzeTailComboTransitions(sourceRow, 70, allBalls);
  ...
  // 5. 🆕 部分匹配模式（保留2-3个尾号是最常见模式） - 已禁用（用户要求去掉）
  // if (allBalls && sourceRow > 0) {
  //   const partialMatchScores = analyzePartialTailMatches(sourceTails, sourceRow, allBalls);
  //   partialMatchScores.forEach(([tail, score]) => {
  //     scores.set(tail, scores.get(tail) + score * weights.partialMatch / 10);
  //   });
  // }

  // 6. 🆕 高频被漏球尾号加成 - 已禁用（用户要求去掉）
  // if (allBalls && sourceRow > 0) {
  //   const highFreqMissTails = analyzeHighFreqMissTails(sourceRow, allBalls);
  //   highFreqMissTails.forEach(tail => {
  //     scores.set(tail, scores.get(tail) + weights.highFreqMiss);
  //   });
  // }

  // 7. 🆕 前一期综合规则：相同或相邻尾号加成（85.6%概率）
  if (allBalls && sourceRow > 0) {
    const sameOrNeighborScores = analyzeSameOrNeighborTails(sourceRow, allBalls);
  ...
  // 14. 桥接尾号加分
  if (sourceTails.length >= 2) {
    const bridgeTailScores = getBridgeTailScores(sourceTails);
    bridgeTailScores.forEach(([tail, score]) => {
      scores.set(tail, scores.get(tail) + score * weights.bridgeTails / 10);
    });
  }

  // 15. 等差延伸邻号（公差2，93.6%命中率，最强信号）
  if (allBalls && sourceRow > 0) {
  ...
  // 9. 🆕 温和频率权重调整（不进行高估/低估处理）
  // 基于历史频率进行温和调整，避免过度干预
  // 平均出现次数 = (56+77+85+84+78+80+65+54+72+72)/10 = 72.3次
  // 平均出现频率 = 72.3/181 = 39.9%
  const tailFrequencyMultipliers = {
    0: 0.90,  // 56/181=30.9% / 39.9% = 0.77 → 温和调整
    1: 1.05,  // 77/181=42.5% / 39.9% = 1.07 → 温和调整
    2: 1.10,  // 85/181=46.9% / 39.9% = 1.18 → 温和调整
    3: 1.10,  // 84/181=46.4% / 39.9% = 1.16 → 温和调整
    4: 1.05,  // 78/181=43.1% / 39.9% = 1.08 → 温和调整
    5: 1.05,  // 80/181=44.2% / 39.9% = 1.11 → 温和调整
    6: 0.95,  // 65/181=35.9% / 39.9% = 0.90 → 温和调整
    7: 0.85,  // 54/181=29.8% / 39.9% = 0.75 → 温和调整
    8: 1.00,  // 72/181=39.8% / 39.9% = 1.00 → 温和调整
    9: 1.00   // 72/181=39.8% / 39.9% = 1.00 → 温和调整
  };

  // 应用频率权重（使用温和的权重）
  const FREQUENCY_WEIGHT = 0.10; // 10%的权重来自历史频率
  ...
  // ✅ 移除多样性强制机制（原奇偶平衡while循环），让分数自然排序
  // 原因：强制平衡扭曲了自然分布，将分数较低的偶数尾号人为提升
  const sortedScores = [...scores.entries()].sort((a, b) => b[1] - a[1]);

  // 🆕 组合优化：从 top8 尾号中选择最符合历史规律的 5 个组合
  // 基于历史频率动态调整权重：常见模式降低权重，不常见但高价值模式提高权重
  ...
  // 返回结果：前 5 个是最佳组合，后面是其他尾号按分数排序
  const result = [];
  bestCombo.forEach(t => { result.push([t, scores.get(t)]); });
  allSorted.forEach(([t, s]) => {
    if (!bestCombo.includes(t)) result.push([t, s]);
  });
...
// ═══ 双模式尾号预测函数（直接在预测阶段分开）═══
// 返回两套独立的尾号预测，而不是从历史数据中分析
function predictLikelyTailsV4DualMode(sourceTails, transData, refRows, sourceRow, allBalls = null) {
  // 基础分数计算（复用原函数的逻辑）- 已整合重叠规则
  const baseWeights = {
    overlap1: 4,                     // 重叠尾号
    sameOrNeighbor: 15,              // 相同或相邻尾号（基线值）
    arith1: 6,
    overlap10: 2,
    arith10: 4,
    overlapBonus: 2,
    ...
    comboTransfer: 2,
    crossRowTail: 10,
    intraGroupPattern: 12,           // 组内尾号规则（基线值）
    arithmeticNumber: 8,
    crossRowArithmetic: 12,
    bridgeTails: 8,                  // 桥接尾号
    tailArithmeticExtension: 12,     // 等差延伸邻号（93.6%命中率）
    tailClustering: 8,
    tailTransition: 10,
    tailPeriodicity: 8,
    tailInterval: 8,
  ...
  for (let t = 0; t <= 9; t++) baseScores.set(t, 0);
  ...
  sourceTails.forEach((st) => {
    for (let tt = 0; tt <= 9; tt++) {
      const key = `${st}→${tt}`;
      const count = transData.transFreq.get(key) || 0;
      baseScores.set(tt, baseScores.get(tt) + count);
    }
  });
  ...
  if (allBalls && sourceRow > 0) {
    ...
    const maxGlobalFreq = Math.max(1, ...globalTailFreq.values());
    globalTailFreq.forEach((count, tail) => {
      baseScores.set(tail, baseScores.get(tail) + (count / maxGlobalFreq) * baseWeights.globalFreq);
    });
  }
  ...
  if (refRows && refRows.length > 0) {
    ...
    if (ref1) {
      ref1.tailSet.forEach(t => {
        baseScores.set(t, baseScores.get(t) + baseWeights.overlap10);
      });
      const arith1 = getArithmeticTails([...ref1.tailSet]);
      arith1.forEach(t => {
        if (!ref1.tailSet.has(t)) {
          baseScores.set(t, baseScores.get(t) + baseWeights.arith1);
        }
      });
    }
    ...
    if (ref10) {
      ...
      arith10.forEach(t => {
        if (!ref10.tailSet.has(t)) {
          baseScores.set(t, baseScores.get(t) + baseWeights.arith10);
        }
      });
    }
    ...
    if (ref1 && ref10) {
      const overlap = [...ref1.tailSet].filter(t => ref10.tailSet.has(t));
      overlap.forEach(t => {
        baseScores.set(t, baseScores.get(t) + baseWeights.overlapBonus);
      });
    }
  }
  ...
  if (allBalls && sourceRow > 0) {
    ...
    const comboTransferScores = predictTailsFromComboTransfer(sourceTails, comboTransData);
    comboTransferScores.forEach(([tail, score]) => {
      baseScores.set(tail, baseScores.get(tail) + score * baseWeights.comboTransfer / 10);
    });
  }
  ...
  if (allBalls && sourceRow > 0) {
    const sameOrNeighborScores = analyzeSameOrNeighborTails(sourceRow, allBalls);
    sameOrNeighborScores.forEach(([tail, score]) => {
      baseScores.set(tail, baseScores.get(tail) + score * baseWeights.sameOrNeighbor / 10);
    });
  }
  ...
  if (allBalls && sourceRow > 2) {
    const crossRowTailScores = analyzeCrossRowTails(sourceRow, allBalls);
    crossRowTailScores.forEach(([tail, score]) => {
      baseScores.set(tail, baseScores.get(tail) + score * baseWeights.crossRowTail / 10);
    });
  }
  ...
  if (allBalls && sourceRow > 0) {
    ...
    if (prevNumbers.length === 5) {
      ...
      arithNumbers.forEach(num => {
        const tail = num % 10;
        baseScores.set(tail, baseScores.get(tail) + baseWeights.arithmeticNumber / 10);
      });
    }
  }
  ...
  if (allBalls && sourceRow > 1) {
    ...
    const prevNumbers = allBalls.filter(b => b.row === prevRow && b.zone === "front").map(b => b.number);
    if (prevNumbers.length === 5) {
      const sortedScores = [...baseScores.entries()].sort((a, b) => b[1] - a[1]);
      const top5Tails = sortedScores.slice(0, 5).map(([tail]) => tail);
      const possibleNumbers = [];
      for (let num = 1; num <= 35; num++) {
        if (top5Tails.includes(num % 10)) {
      ...
      crossArithNumbers.forEach(num => {
        const tail = num % 10;
        baseScores.set(tail, baseScores.get(tail) + baseWeights.crossRowArithmetic / 10);
      });
    }
  }
  ...
  if (sourceTails.length >= 2) {
    const bridgeTailScores = getBridgeTailScores(sourceTails);
    bridgeTailScores.forEach(([tail, score]) => {
      baseScores.set(tail, baseScores.get(tail) + score * baseWeights.bridgeTails / 10);
    });
  }

  // 10. 等差延伸邻号（公差2，93.6%命中率）
  if (allBalls && sourceRow > 0) {
    const arithExtScores = analyzeTailArithmeticExtension(sourceRow, allBalls);
    arithExtScores.forEach(([tail, score]) => {
      baseScores.set(tail, baseScores.get(tail) + score * baseWeights.tailArithmeticExtension / 10);
    });
  }
  ...
  // 模式1: 等差+连续+连接点预测
  const arithConsecScores = new Map(baseScores); // 复制基础分数
  if (allBalls && sourceRow > 0) {
    // 分析历史等差+连续+连接点模式
    const arithConsecPatterns = analyzeArithConsecPatterns(sourceRow, allBalls);
    arithConsecPatterns.forEach(([tail, score]) => {
      arithConsecScores.set(tail, arithConsecScores.get(tail) + score * 15); // 高权重
    });
    
    // 等差延伸邻号（针对等差模式）
    const arithExtScores = analyzeTailArithmeticExtension(sourceRow, allBalls);
    arithExtScores.forEach(([tail, score]) => {
      arithConsecScores.set(tail, arithConsecScores.get(tail) + score * 10);
    });
    
    // 连续尾号加成
    const consecutiveScores = analyzeConsecutiveTails(sourceRow, allBalls);
    consecutiveScores.forEach(([tail, score]) => {
      arithConsecScores.set(tail, arithConsecScores.get(tail) + score * 12);
    });
  }

  // 模式2: 多段连续+连接点预测
  const multiSegScores = new Map(baseScores); // 复制基础分数
  if (allBalls && sourceRow > 0) {
    // 分析历史多段连续+连接点模式
    const multiSegPatterns = analyzeMultiSegPatterns(sourceRow, allBalls);
    multiSegPatterns.forEach(([tail, score]) => {
      multiSegScores.set(tail, multiSegScores.get(tail) + score * 15); // 高权重
    });
    
    // 多段连续特有信号
    const multiSegSpecific = analyzeMultiSegmentSpecific(sourceRow, allBalls);
    multiSegSpecific.forEach(([tail, score]) => {
      multiSegScores.set(tail, multiSegScores.get(tail) + score * 12);
    });
  }

  // 生成两套独立的Top5预测
  const arithConsecSorted = [...arithConsecScores.entries()].sort((a, b) => b[1] - a[1]);
  const arithConsecTails = arithConsecSorted.slice(0, 5).map(([t]) => t);

  const multiSegSorted = [...multiSegScores.entries()].sort((a, b) => b[1] - a[1]);
  const multiSegTails = multiSegSorted.slice(0, 5).map(([t]) => t);

  // 返回结果
  return {
    arithConsecTails,
    multiSegTails,
    arithConsecScores: arithConsecSorted,
    multiSegScores: multiSegSorted,
    baseScores: [...baseScores.entries()].sort((a, b) => b[1] - a[1]),
  };
}

// ═══ 分析等差+连续+连接点模式 ═══
function analyzeArithConsecPatterns(sourceRow, allBalls) {
  const tailScores = new Map();
  for (let t = 0; t <= 9; t++) tailScores.set(t, 0);
  ...
  for (let r = start; r < sourceRow; r++) {
    ...
    // 等差+连续+连接点模式
    if (hasArithmetic && maxConsecutive >= 2) {
      let hasAnchor = false;
      for (const t of arithTails) {
        if (isAnchor(t)) { hasAnchor = true; break; }
      }
      if (!hasAnchor) {
      ...
      if (hasAnchor) {
        ...
        uniqueTails.forEach(tail => {
          tailScores.set(tail, tailScores.get(tail) + 1);
        });
      }
    }
  }
  ...
  // 归一化
  const maxScore = Math.max(1, ...tailScores.values());
  tailScores.forEach((count, tail) => {
    tailScores.set(tail, (count / maxScore) * 10);
  });
...
// ═══ 分析多段连续+连接点模式 ═══
function analyzeMultiSegPatterns(sourceRow, allBalls) {
  const tailScores = new Map();
  for (let t = 0; t <= 9; t++) tailScores.set(t, 0);
  ...
  for (let r = start; r < sourceRow; r++) {
    ...
    if (segments.length >= 2) {
      ...
      for (const seg of segments) {
        ...
        if (isAnchor(startTail) || isAnchor(endTail)) {
          hasAnchor = true;
          break;
        }
      }
...
// ═══ 分析连续尾号 ═══
function analyzeConsecutiveTails(sourceRow, allBalls) {
  const tailScores = new Map();
  for (let t = 0; t <= 9; t++) tailScores.set(t, 0);
  ...
  for (let r = start; r < sourceRow; r++) {
    ...
    for (let i = 0; i < sortedTails.length - 1; i++) {
      if (sortedTails[i+1] === sortedTails[i] + 1) {
        tailScores.set(sortedTails[i], tailScores.get(sortedTails[i]) + 1);
        tailScores.set(sortedTails[i+1], tailScores.get(sortedTails[i+1]) + 1);
      }
    }
  }
...
// ═══ 分析多段连续特有信号 ═══
function analyzeMultiSegmentSpecific(sourceRow, allBalls) {
  const tailScores = new Map();
  for (let t = 0; t <= 9; t++) tailScores.set(t, 0);
...
// ═══ 分析部分匹配模式（保留2-3个尾号）═══
function analyzePartialTailMatches(sourceTails, sourceRow, allBalls) {
  const tailScores = new Map();
  for (let t = 0; t <= 9; t++) tailScores.set(t, 0);
...
// ═══ 分析高频被漏球尾号 ═══
function analyzeHighFreqMissTails(sourceRow, allBalls) {
...
function analyzeSameOrNeighborTails(sourceRow, allBalls) {
  const tailScores = new Map();
  for (let t = 0; t <= 9; t++) tailScores.set(t, 0);
...
function analyzeCrossRowTails(sourceRow, allBalls) {
  ...
  for (let r = start; r < sourceRow; r++) {
    const srcNums = [...new Set(ballsByRow.get(r) || [])];      // 源行（前一期）
    const tgtNums = [...new Set(ballsByRow.get(r + 1) || [])];  // 目标行
  ...
  if (sourceRow > 1) {
    ...
    if (ref1 && ref1.length === 5) {
      ...
      ref1Tails.forEach(tail => {
        tailScores.set(tail, tailScores.get(tail) + 3); // 前1期加分
      });
    }
  }
...
// ═══ 基于组内尾号规则+跨期尾号规则的尾号预测（简化版）═══
// 只使用这两个规则，不使用转移概率、全局频率等复杂因素
function predictTailsByGroupAndCrossRowRules(sourceRow, allBalls) {
  const scores = new Map();
  for (let t = 0; t <= 9; t++) scores.set(t, 0);

  // 1. 组内尾号规则：分析历史尾号组合模式（多样性+连续性+等差性）
  if (sourceRow > 0) {
    const intraGroupScores = analyzeIntraGroupTailPatterns(sourceRow, allBalls);
    intraGroupScores.forEach(([tail, score]) => {
      scores.set(tail, scores.get(tail) + score * 2.0); // 组内规则权重×2.0
    });
  }

  // 2. 跨期尾号规则：与前2-3期尾号重复规律
  if (sourceRow > 2) {
    const crossRowScores = analyzeCrossRowTails(sourceRow, allBalls);
    crossRowScores.forEach(([tail, score]) => {
      scores.set(tail, scores.get(tail) + score * 1.5); // 跨期规则权重×1.5
    });
  }
  ...
  if (sourceRow > 0) {
    const tailFreq = new Map();
    for (let t = 0; t <= 9; t++) tailFreq.set(t, 0);
    const lookback = Math.min(30, sourceRow);
    const ballsByRow = new Map();
    allBalls.filter(b => b.zone === "front").forEach(b => {
      if (!ballsByRow.has(b.row)) ballsByRow.set(b.row, []);
      ballsByRow.get(b.row).push(b.number);
    ...
    for (let r = Math.max(1, sourceRow - lookback); r < sourceRow; r++) {
      const nums = [...new Set(ballsByRow.get(r) || [])];
      if (nums.length === 5) {
        nums.forEach(n => tailFreq.set(n % 10, tailFreq.get(n % 10) + 1));
      }
    }
    const maxFreq = Math.max(1, ...tailFreq.values());
    tailFreq.forEach((count, tail) => {
      scores.set(tail, scores.get(tail) + (count / maxFreq) * 5); // 频率权重×5
    });
  }
...
function predictFirstBallComprehensive(sourceRow, allBalls) {
  ...
  // 基础分：历史频率（近20期）- 优化：预分组避免15次filter
  const _fbBr = new Map();
  allBalls.filter(b => b.zone === "front").forEach(b => {
...
const V4_POOL_SIZE = 30; // 最优池大小（Top5命中率峰值34.4%，三区覆盖72.4%）
function buildSampleNumbersV4(selectedRow, zone, ratios, _preCollected) {
  ...
  // 🆕 v6: 分区间独立预测（拆分一区/二区/三区独立分析方向）
  const zoneDirections = predictZoneDirections(sourceRow, allBalls);
  ...
  for (let n = 1; n <= 35; n++) {
    ...
    if (extremeFlags.narrowRange && minOffset >= 2) { score += 4; extremeTriggered = true; }
    if (extremeFlags.sumCrash && minOffset >= 3) { score += 5; extremeTriggered = true; }
    if (extremeFlags.parityFlip && n % 2 !== selectedNumbers[0] % 2) { score += 3; extremeTriggered = true; }
    if (extremeTriggered) signals.push('extreme');
    ...
    if (n <= 15) {
      ...
      if (rank >= 0 && rank < 5) { score += 12; signals.push('first_ball'); }       // Top5预测号码
    ...
    if (n === 35) {
      ...
      if (hasZone1Anchor) { score += 10; specialTriggered = true; }  // 🆕 优化：有一区锚点时加分（8→10）
      // 🆕 补充：球35在三区最高位，需要额外基础分
      score += 3;
    }
    // 球30与球29（连续号）有强关联
    if (n === 30) {
      if (selectedNumbers.includes(29)) { score += 12; specialTriggered = true; }  // 🆕 优化：有29时强加分（10→12）
      if (selectedNumbers.includes(28) || selectedNumbers.includes(31)) { score += 6; specialTriggered = true; }  // 🆕 优化：有相邻号时加分（5→6）
    }
    // 球31频率最低，需要额外加分
    if (n === 31) {
      if (selectedNumbers.includes(30) || selectedNumbers.includes(32)) { score += 8; specialTriggered = true; }  // 🆕 优化：相邻号加分（6→8）
    }
    // 球25频率偏低，需要额外加分
    if (n === 25) {
      if (selectedNumbers.includes(24) || selectedNumbers.includes(26)) { score += 7; specialTriggered = true; }  // 🆕 优化：相邻号加分（5→7）
    }
    // 🆕 新增：球26高频被漏（31次），需要额外加分
    if (n === 26) {
      if (selectedNumbers.includes(25) || selectedNumbers.includes(27)) { score += 6; specialTriggered = true; }  // 有相邻号时加分
      if (selectedNumbers.some(x => x >= 15 && x <= 20)) score += 3;  // 有二区锚点时加分
    }
    // 🆕 新增：球27高频被漏（19次），需要额外加分
    if (n === 27) {
      if (selectedNumbers.includes(26) || selectedNumbers.includes(28)) { score += 5; specialTriggered = true; }  // 有相邻号时加分
    }
    if (specialTriggered) signals.push('special');
    ...
    // 测试结果：协同加分+16 命中3+提升100%（2→4期），命中4+从0→1期
    {
  ...
  const minIv = [3, 3, 4]; // 三区保底保持4（测试纯池大小效果）
  if (sourceRow >= 3) {
    ...
    if (prev1Nums.length === 5) {
      ...
      // 🆕 优化：三区极高(>=4)：100%回归，但保底不低于3（原2→3）
      if (prev1Iv[2] >= 4) minIv[2] = 3;
    }
      ...
      for (let z = 0; z < 3; z++) {
        ...
        if (change1 > 0 && change2 > 0) {
          if (z === 1) {
            ...
            // 🆕 优化：三区100%减小，但保底不低于3（原2→3）
            minIv[z] = Math.max(3, minIv[z] - 1);
          } else {
            // 一区78%减小
  ...
  if (predictedTails && predictedTails.length > 0) {
    // 只取Top5预测尾号，不加邻号尾号（提高聚焦性）
    predictedTails.slice(0, 5).forEach(([tail]) => predictedTailSet.add(tail));
  }
  ...
  return {
    ...
    candidateEntries,
    _allCandidates: candidates, // 🆕 诊断：完整35球排名（含分数）
    numbers: candidateEntries.slice(0, 5).map((e) => e.number),
    predictedTails, // 🆕 尾号预测结果（供v4.1组合评分使用）
    ivPrediction,  // 🆕 v5: 区间比预测（供组合评分IV微调使用）
    zoneDirections, // 🆕 v6: 分区间独立方向预测（供候选评分和保底修正使用）
    extremeFlags,  // 🆕 极端期检测标志（供组合评分使用）
    firstBallPredictions, // 🆕 首位球预测（供下游复用，避免重复计算）
  };
}

// ═══ 候选号码评分实验：去掉候选号码评分，直接使用所有1-35号码 ═══
function buildSampleNumbersV4NoScoring(selectedRow, zone, ratios, _preCollected) {
  zone = zone || "front";
  const sourceColor = sampleRedColor;
  const allBalls = _preCollected || collectBalls();
  const sourceWindow = getSampleSourceWindow(selectedRow);
  ...
  // 直接使用所有1-35号码作为候选池，不进行评分排序
  const candidates = [];
  for (let n = 1; n <= 35; n++) {
    candidates.push({ number: n, score: 1, signals: [] }); // 统一分数为1
  }
  ...
  return {
    ...
    zoneDirections,
    extremeFlags,
    firstBallPredictions,
  };
}

// ═══ 候选号码评分实验：只使用预测尾号+邻号构建候选池 ═══
function buildSampleNumbersV4TailOnly(selectedRow, zone, ratios, _preCollected) {
  zone = zone || "front";
  const sourceColor = sampleRedColor;
  const allBalls = _preCollected || collectBalls();
  const sourceWindow = getSampleSourceWindow(selectedRow);
  ...
  // ═══ 🆕 实验：差异化评分候选池构建 ═══
  const predictedTailSet = new Set();
  const tailScoreMap = new Map(); // 尾号→预测分数
  const TAIL_PREDICT_TOP_N = 5;
  
  if (predictedTails && predictedTails.length > 0) {
    // 🆕 计算分数范围用于归一化
    const maxScore = predictedTails[0][1] || 1;
    const minScore = predictedTails[Math.min(TAIL_PREDICT_TOP_N - 1, predictedTails.length - 1)][1] || 0;
    const scoreRange = Math.max(1, maxScore - minScore);
    
    // 只取TopN预测尾号，不加邻号尾号（提高聚焦性）
    predictedTails.slice(0, TAIL_PREDICT_TOP_N).forEach(([tail, score]) => {
      predictedTailSet.add(tail);
      // 归一化分数：Top1=100, Top5=50
      const normalizedScore = 100 - (score - minScore) / scoreRange * 50;
      tailScoreMap.set(tail, normalizedScore);
    });
  }

  // 收集所有尾号匹配的号码（1-35），差异化评分
  const candidates = [];
  for (let n = 1; n <= 35; n++) {
    const t = n % 10;
    if (predictedTailSet.has(t)) {
      const baseScore = tailScoreMap.get(t) || 50;
      // 🆕 额外加分：源号码本身
      const isSource = selectedNumbers.includes(n);
      const finalScore = isSource ? baseScore * 1.2 : baseScore;
      candidates.push({ number: n, score: Math.round(finalScore), signals: isSource ? ['tail_match', 'source'] : ['tail_match'] });
    }
  }

  // 如果候选池太小，添加源号码本身
  for (const n of selectedNumbers) {
    if (!candidates.some(c => c.number === n)) {
      candidates.push({ number: n, score: 30, signals: ['source_number'] });
    }
  }
      ...
      for (let c = b + 1; c < topN.length - 2 && combos.length < loopLimit; c++) {
        for (let d = c + 1; d < topN.length - 1 && combos.length < loopLimit; d++) {
          for (let e = d + 1; e < topN.length && combos.length < loopLimit; e++) {
            ...
            // 🆕 组合生成阶段：桥接、相邻、重复关系加分
            var tails = Array.from(new Set(nums.map(function(n) { return n % 10; })));
            var sortedTails = tails.slice().sort(function(a, b) { return a - b; });
            
            // 1. 组合内连续尾号加分（两个连续最常见，分值最高）
            var maxConsec = 1, curConsec = 1;
            for (var ti = 1; ti < sortedTails.length; ti++) {
              if (sortedTails[ti] === sortedTails[ti-1] + 1 || (sortedTails[ti-1] === 9 && sortedTails[ti] === 0)) {
                curConsec++;
            ...
            if (maxConsec >= 2) score += 12;  // 两个连续最常见，分值最高
            else if (maxConsec >= 3) score += 10;
            ...
            for (var td = 2; td <= 4; td++) {
              for (var ts = 0; ts <= 9 - td * 2; ts++) {
                ...
                if (tcnt >= 3) { hasAP = true; break; }
              }
              if (hasAP) break;
            }
            if (hasAP) score += 6;
            
            // 2.1 组合内差值递增加分（如1,4,8：差值3,4递增）
            if (sortedTails.length >= 3) {
              var diffs = [];
              for (var di = 1; di < sortedTails.length; di++) {
                diffs.push(sortedTails[di] - sortedTails[di-1]);
              }
              var isIncreasing = true;
              for (var di = 1; di < diffs.length; di++) {
                if (diffs[di] <= diffs[di-1]) {
                  isIncreasing = false;
                  break;
                }
              }
              if (isIncreasing) {
                score += 8;
                if (typeof _diffIncCount !== 'undefined') _diffIncCount++;
              }
            }
            
            // 3. 跨期关系（需要获取前一期数据）
            if (typeof rows !== 'undefined' && typeof getBallsAtRow === 'function') {
              var prevRow = rows - 1;
              var prevBalls = getBallsAtRow(prevRow, 'front');
              if (prevBalls && prevBalls.length > 0) {
                var prevTails = Array.from(new Set(prevBalls.map(function(b) { return b.number % 10; })));
                
                // 3.1 重复尾号加分
                var repeatCount = tails.filter(function(t) { return prevTails.includes(t); }).length;
                if (repeatCount >= 3) score += 10;
                else if (repeatCount >= 2) score += 6;
                else if (repeatCount >= 1) score += 3;
                ...
                if (neighborCount >= 3) score += 8;
                else if (neighborCount >= 2) score += 4;
                
                // 3.3 等差关系加分（与前一期尾号等差）
                var apCount = 0;
                tails.forEach(function(t) {
                  prevTails.forEach(function(pt) {
                    var diff = Math.abs(t - pt);
                    if (diff >= 2 && diff <= 4) apCount++;
                  });
                });
                if (apCount >= 3) score += 6;
...
function v5GenCombosRandom(dimPool, count, maxPool, weightPower, options = {}) {
  maxPool = maxPool || 13;
  weightPower = weightPower || 0.5;
  var sourceMap = options.sourceMap || null;
  var bridgeMap = options.bridgeMap || null;
  var selectedNumbers = options.selectedNumbers || [];
  var lastRowBalls = options.lastRowBalls || [];
  var arithMap = options.arithMap || null;
  var selectedSet = new Set(selectedNumbers);
  var lastRowSet = new Set(lastRowBalls.map(function(b) { return b.number || b; }));
  var lastRowTailSet = new Set(lastRowBalls.map(function(b) { return (b.number || b) % 10; }));
  var lastRowTailNeighborSet = buildTailNeighborSet(lastRowBalls.map(function(b) { return (b.number || b) % 10; }));
  ...
  for (var attempt = 0; attempt < maxAttempts && combos.length < count * 10; attempt++) {
    ...
    // 2. 尾号等差（tailArithmetic）
    var tailArithmetic = buildSampleTailArithmeticProfile(nums);
    
    // 3. 尾号支持（tailSupport）
    var tailSupport = 0;
    if (sourceMap) {
      nums.forEach(function(number) {
        var entry = sourceMap.get(number);
        if (entry) tailSupport += Math.min(2, entry.tailCount || 0);
      });
    }
    
    // 4. 桥接间隔命中（bridgeGapHits）和桥接端点命中（bridgeEndpointHits）
    var bridgeGapHits = 0, bridgeEndpointHits = 0;
    if (bridgeMap) {
      nums.forEach(function(number) {
        var bg = bridgeMap.gapMap.get(number);
        var be = bridgeMap.endpointMap.get(number);
        if (bg) bridgeGapHits += bg.hits || 0;
        if (be) bridgeEndpointHits += be.hits || 0;
      });
    }
    
    // 5. 等差端点命中（arithmeticEndpointHits）
    var arithmeticEndpointHits = 0;
    if (arithMap) {
      nums.forEach(function(number) {
        var ae = arithMap.get(number);
        if (ae) arithmeticEndpointHits += ae.hits || 0;
      });
    }
    
    // 6. 与选中行重叠（selectedOverlap）
    var selectedOverlap = nums.filter(function(number) { return selectedSet.has(number); }).length;
    
    // 7. 与选中行相邻（selectedNeighborHits）
    var selectedNeighborHits = 0;
    nums.forEach(function(number) {
      if (selectedSet.has(number - 1) || selectedSet.has(number + 1)) selectedNeighborHits += 1;
    });
    
    // 8. 连续对（consecutivePairs）和最长连续（longestRun）
    var consecutiveResult = countSampleConsecutivePairs(nums);
    var consecutivePairs = consecutiveResult.pairs;
    var longestRun = consecutiveResult.longestRun;
    
    // 9. 连续段（consecutiveSegments）
    var consecutiveSegments = buildSampleConsecutiveSegments(nums);
    
    // 10. 与上期重叠（lastRowOverlap）
    var lastRowOverlap = nums.filter(function(number) { return lastRowSet.has(number); }).length;
    
    // 11. 与上期相邻（lastRowNeighborHits）
    var lastRowNeighborHits = 0;
    nums.forEach(function(number) {
      if (lastRowSet.has(number - 1) || lastRowSet.has(number + 1)) lastRowNeighborHits += 1;
    });
    
    // 12. 重复支持（repeatSupport）
    var repeatSupport = 0;
    if (sourceMap) {
      nums.forEach(function(number) {
        var entry = sourceMap.get(number);
        if (entry) repeatSupport += entry.repeatCount || 0;
      });
    }
    
    // 13. 跨期等差关系（hasCrossAP）
    var hasCrossAP = false;
    if (lastRowBalls.length > 0) {
      var prevTails = Array.from(new Set(lastRowBalls.map(function(b) { return (b.number || b) % 10; })));
      var allTails = Array.from(new Set(prevTails.concat(tails))).sort(function(a,b){return a-b;});
      for (var td = 1; td <= 9 && !hasCrossAP; td++) {
        for (var ts = 0; ts <= 9 && !hasCrossAP; ts++) {
          var tcnt = 0;
          for (var tv = ts; tv <= 9; tv += td) {
            if (allTails.includes(tv)) tcnt++;
            else break;
          }
          if (tcnt >= 3) hasCrossAP = true;
        }
      }
    }
    
    // 计算总分（使用sampleSignalLevel和sampleRuleWeight）
    score = 
      sampleSignalLevel(tailMatches, 3) * (sampleRuleWeight + 2) +
      sampleSignalLevel(tailArithmetic.strongestCount - 1, 3) * sampleRuleWeight +
      sampleSignalLevel(tailSupport, 2) * sampleRuleWeight +
      sampleSignalLevel(bridgeGapHits, 3) * sampleRuleWeight +
      sampleSignalLevel(bridgeEndpointHits, 3) * sampleRuleWeight +
      ...
      sampleSignalLevel(consecutivePairs, 3) * sampleRuleWeight +
      sampleSignalLevel(longestRun - 1, 3) * sampleRuleWeight +
      sampleSignalLevel(consecutiveSegments.filter(function(segment) { return segment.length === 2; }).length, 3) * sampleRuleWeight +
      sampleSignalLevel(lastRowOverlap, 3) * sampleRuleWeight +
      sampleSignalLevel(lastRowNeighborHits, 3) * sampleRuleWeight +
      sampleSignalLevel(repeatSupport, 3) * sampleRuleWeight +
      (hasCrossAP ? 10 : 0);
  ...
  return combos.map(function (c) {
    ...
    if (typeof rows !== 'undefined' && typeof getBallsAtRow === 'function') {
      ...
      if (prevBalls && prevBalls.length > 0) {
        ...
        // 1. 跨期连续关系（相同尾号或相邻尾号）
        var crossConsec = 0;
        comboTailsArr.forEach(function(t) {
          if (prevTails.includes(t)) crossConsec += 2; // 相同尾号
          else if (prevTails.some(function(pt) { 
            return Math.abs(pt - t) === 1 || Math.abs(pt - t) === 9; // 相邻（含0-9循环）
          })) crossConsec += 1;
        });
        ...
        comboTailsArr.forEach(function(t) {
          prevTails.forEach(function(pt) {
            var diff = Math.abs(t - pt);
            if (diff >= 2 && diff <= 4) crossAP++;
          });
        });
        if (crossAP >= 4) bonus += 6;
        else if (crossAP >= 2) bonus += 3;

        // 3. 跨行尾号重复（与前2-3期的尾号重复）
        for (var lookback = 2; lookback <= 3; lookback++) {
          var prevRowLB = rows - lookback;
          var prevBallsLB = getBallsAtRow(prevRowLB, 'front');
          if (prevBallsLB && prevBallsLB.length > 0) {
            var prevTailsLB = Array.from(new Set(prevBallsLB.map(function(b) { return b.number % 10; })));
            var repeatCount = comboTailsArr.filter(function(t) { return prevTailsLB.includes(t); }).length;
...
function buildSampleFrontCombosV5(entries, refs, anchorNumbers, selectedNumbers, predictTails, ivPrediction, firstBallPredictions, extremeFlags, manualRatio, sourceRow, _preCollected) {
  ...
  // 🆕 优化：移除重复维度（重复号码、跨期关系），将其移至完整评分阶段统一处理
  // 避免尾号、重复号码、跨期关系在两个阶段都被评分导致过度偏向
  var dims = [
  ...
  if (typeof rows !== 'undefined' && typeof getBallsAtRow === 'function') {
    var prevRow = rows - 1;
    lastRowBalls = getBallsAtRow(prevRow, 'front') || [];
  }
  var comboOptions = {
    bridgeMap: bridgeMap,
    selectedNumbers: selectedNumbers,
    lastRowBalls: lastRowBalls,
    arithMap: arithMap,
    sourceMap: null // sourceMap暂不可用
  };
  dims.forEach(function (d) {
    var wp = 0.5;  // weightPower固定值（随机化测试未提升）
    v5GenCombosRandom(d.arr, d.count, d.maxPool, wp, comboOptions).forEach(function (c) {
  ...
  if (allCombos.length < 8) {
    ...
    v5GenCombosRandom(suppArr, 8 - allCombos.length, 13, 0.5, comboOptions).forEach(function (c) {
      if (!usedKeys.has(c.key)) { allCombos.push(c); usedKeys.add(c.key); }
    });
  }
...
// ═══ v4.1 单组五行计划（双模式策略：等差+连续+连接点×3 + 多段连续+连接点×3）═══
function buildV4SingleSamplePlan(frontCombos, backCombos, poolNumbers = null, candidateEntries = null, predictedTails = null, firstBallPreds = null, targetPatterns = null, sourceRow = null, allBalls = null, arithConsecTails = [], multiSegTails = []) {
  const frontBase = frontCombos[0] || null;
  const backBase = backCombos[0] || null;
  if (!frontBase && !backBase) return [];
  ...
  // 🆕 提取源号码用于号码对关系评分
  let sourceNums = [];
  if (sourceRow && allBalls) {
    const srcBalls = allBalls.filter(b => b.zone === "front" && b.row === sourceRow);
    sourceNums = [...new Set(srcBalls.map(b => b.number))].sort((a, b) => a - b);
  }
  
  // 🔍 调试：输出源号码和双模式尾号数据
  console.log(`[调试] buildV4SingleSamplePlan 被调用`);
  console.log(`[调试] sourceRow: ${sourceRow}, allBalls长度: ${allBalls?.length || 0}`);
  console.log(`[调试] sourceNums: [${sourceNums.join(', ')}]`);
  console.log(`[调试] arithConsecTails: [${arithConsecTails.join(', ')}]`);
  console.log(`[调试] multiSegTails: [${multiSegTails.join(', ')}]`);
  
  // 覆盖优先策略：选择Top5使得5组联合覆盖尽可能多的候选池号码
  // 构建尾号关系数据
  let tailRelationData = null;
  if (targetPatterns && sourceRow && allBalls) {
    const mainSourceTails = [...new Set(sourceNums.map(n => n % 10))];
    const prevRow = sourceRow - 1;
  ...
  // ═══ 双模式策略选择Top6 = 等差+连续+连接点(3) + 多段连续+连接点(3) ═══
  ...
  function calcNumberPairScore(nums, sourceNums) {
    if (!nums || nums.length === 0 || !sourceNums || sourceNums.length === 0) {
      console.log(`[调试] calcNumberPairScore 返回0，nums长度:${nums?.length || 0}, sourceNums长度:${sourceNums?.length || 0}`);
      return 0;
    }
    ...
    score += Math.min(repeatCount, 3) * 5; // 最多3个重复，每个+5
    ...
    score += Math.min(consecutivePairs, 2) * 6; // 最多2对连号，每对+6
    ...
    score += Math.min(bridgeCount, 2) * 4; // 最多2个桥接，每个+4
    ...
    for (const n of sorted) {
      for (const src of srcSorted) {
        ...
        if (diff >= 2 && diff <= 5 && diff !== 3) { // 等差距离2-5（排除3，太常见）
          arithmeticCount++;
          break;
        }
      }
    }
    score += Math.min(arithmeticCount, 3) * 3; // 最多3个等差号，每个+3
    
    console.log(`[调试] calcNumberPairScore 计算完成，组合:[${nums.join(',')}], sourceNums长度:${sourceNums.length}, 得分:${score}`);
    return score;
  }
  
  // 辅助函数：从候选组合中选出最匹配指定尾号集合的组合
  function selectCombosByTails(tails, count, combos, usedKeysSet) {
    console.log(`[调试] selectCombosByTails 被调用，tails: [${tails.join(', ')}], count: ${count}, combos数量: ${combos.length}`);
    ...
    const scored = combos.map(combo => {
      ...
      // 🆕 加入号码对关系评分
      const pairScore = calcNumberPairScore(nums, sourceNums);
      // 调试：显示前3个组合的评分
      if (debugCount < 3) {
        console.log(`    [调试] 组合 [${nums.join(',')}] 尾号匹配:${matchScore} 号码对:${pairScore} 原始分:${(combo.weightedScore || combo.score || 0) * 0.1}`);
        debugCount++;
      }
      return { ...combo, modeScore: matchScore + pairScore + (combo.weightedScore || combo.score || 0) * 0.1 };
    });
    ...
    console.log(`[调试] selectCombosByTails 返回 ${result.length} 个组合`);
    return result;
  }
  ...
  // 第二步：从"多段连续+连接点"模式选3个最优组合
  if (multiSegTails.length > 0 && selectedFront.length < 6) {
    const need = 6 - selectedFront.length;
    const selected = selectCombosByTails(multiSegTails, need, frontCombos, usedKeys);
    selectedFront.push(...selected);
  }
  
  // 第三步：如果仍然不足，用覆盖优先策略补充
  const remaining = 6 - selectedFront.length;
  if (remaining > 0) {
    const coverageSelected = poolNumbers && poolNumbers.length > 0
      ? selectCoverageOptimalCombos(frontCombos, poolNumbers, 6, tailRelationData)
      : frontCombos.slice(0, 6);
    
    for (const c of coverageSelected) {
      if (selectedFront.length >= 6) break;
      const key = c.numbers ? c.numbers.join(',') : c.key;
      if (!usedKeys.has(key)) {
  ...
  // 生成6组方案（双模式Top6）
  const variants = [];
  const modeNames = ['等差①', '等差②', '等差③', '多段①', '多段②', '多段③'];
  for (let offset = 0; offset < Math.min(6, selectedFront.length); offset += 1) {
    const front = selectedFront[offset] || frontBase;
    const back = backCombos[offset % backCombos.length] || backBase;
    variants.push({
      key: `single-${offset + 1}`,
      label: modeNames[offset] || `Top${offset + 1}`,
      front, back,
...
function selectCoverageOptimalCombos(allCombos, poolNumbers, n, tailRelationData = null) {
  ...
  for (let round = 1; round < n && selected.length < n; round++) {
    ...
    for (let i = 1; i < allCombos.length; i++) {
      ...
      // 计算尾号关系加分
      let tailRelationBonus = 0;
      if (tailRelationData) {
        const comboTails = new Set(c.numbers.map(n => n % 10));
        const { predictedTails, sourceTails, prevTails, targetPatterns } = tailRelationData;
        
        // 1. 预测尾号匹配
        if (predictedTails && predictedTails.length > 0) {
          const predSet = new Set(predictedTails.slice(0, 5).map(([t]) => t));
          comboTails.forEach(t => {
            if (predSet.has(t)) tailRelationBonus += 10;
            else if (predictedTails.some(([pt]) => Math.abs(pt - t) === 1)) tailRelationBonus += 5;
          });
        }
        
        // 2. 源尾号关系
        if (sourceTails && sourceTails.length > 0) {
          comboTails.forEach(t => {
            if (sourceTails.includes(t)) tailRelationBonus += 8;
          });
        }
        
        // 3. 前一期尾号关系
        if (prevTails && prevTails.length > 0) {
          comboTails.forEach(t => {
            if (prevTails.includes(t)) tailRelationBonus += 6;
            else if (prevTails.some(pt => Math.abs(pt - t) === 1)) tailRelationBonus += 3;
          });
        }
        
        // 4. 目标号码尾号模式（关联性、分布规律、时序规律）
        if (targetPatterns) {
          const comboTailsArray = [...comboTails];
          
          // 4.1 尾号关联性加分（等差、连续，不限个数）
          {
            const tails = comboTailsArray.sort((a, b) => a - b);
            const prevTails = tailRelationData.prevTails || [];
            
            // 1. 组合内尾号连续检查（至少2个连续）
            let maxConsecutive = 1, curConsecutive = 1;
            for (let i = 1; i < tails.length; i++) {
              if (tails[i] === tails[i-1] + 1 || (tails[i-1] === 9 && tails[i] === 0)) {
                curConsecutive++;
            ...
            if (maxConsecutive >= 4) tailRelationBonus += 6;
            else if (maxConsecutive >= 3) tailRelationBonus += 4;
            else if (maxConsecutive >= 2) tailRelationBonus += 2;
            
            // 2. 组合内等差检查（步长1-4，至少3个元素）
            let hasArithmetic = false;
            for (let step = 1; step <= 4; step++) {
              for (let i = 0; i < tails.length; i++) {
                let count = 1;
                for (let j = i + 1; j < tails.length; j++) {
                  if (tails[j] === tails[i] + step * count) count++;
                }
                if (count >= 3) { hasArithmetic = true; break; }
              }
              if (hasArithmetic) break;
            }
            if (hasArithmetic) tailRelationBonus += 4;
            
            // 3. 与前一期尾号的连续关系
            if (prevTails.length > 0) {
              let crossConsecutive = 0;
              tails.forEach(t => {
                if (prevTails.includes(t)) crossConsecutive += 2; // 相同
                else if (prevTails.some(pt => Math.abs(pt - t) === 1 || Math.abs(pt - t) === 9)) crossConsecutive += 1; // 相邻
              });
              if (crossConsecutive >= 4) tailRelationBonus += 5;
              else if (crossConsecutive >= 2) tailRelationBonus += 3;
              
              // 4. 与前一期尾号的等差关系
              let crossArithmetic = 0;
              tails.forEach(t => {
                prevTails.forEach(pt => {
                  const diff = Math.abs(t - pt);
                  if (diff >= 1 && diff <= 4 && diff !== 1) crossArithmetic++; // 步长2-4
                });
              });
              if (crossArithmetic >= 3) tailRelationBonus += 4;
              else if (crossArithmetic >= 2) tailRelationBonus += 2;
            }
          }
          
          // 4.2 分布规律加分（区间、奇偶、大小分布）
          if (targetPatterns.zoneDistribution) {
            const iv = [0, 0, 0];
            c.numbers.forEach(n => {
              if (n <= 12) iv[0]++;
              else if (n <= 24) iv[1]++;
            ...
            const total = iv.reduce((a, b) => a + b, 0) || 1;
            const ivRatio = iv.map(v => v / total);
            
            // 计算与目标分布的相似度
            let similarity = 0;
            for (let i = 0; i < 3; i++) {
              similarity += Math.min(ivRatio[i], targetPatterns.zoneDistribution[i]);
            }
            if (similarity >= 0.7) tailRelationBonus += 4;
            else if (similarity >= 0.5) tailRelationBonus += 2;
          }
          
          if (targetPatterns.oddEvenDistribution) {
            const odd = c.numbers.filter(n => n % 2 === 1).length;
            const even = c.numbers.length - odd;
            const total = c.numbers.length || 1;
            const oddRatio = odd / total;
            const evenRatio = even / total;
            
            const similarity = Math.min(oddRatio, targetPatterns.oddEvenDistribution[0]) + 
                              Math.min(evenRatio, targetPatterns.oddEvenDistribution[1]);
            if (similarity >= 0.8) tailRelationBonus += 3;
            else if (similarity >= 0.6) tailRelationBonus += 1;
          }
          
          // 4.3 时序规律加分（尾号变化模式）
          if (targetPatterns.tailChangeFreq && prevTails && prevTails.length > 0) {
            let sameCount = 0, neighborCount = 0, farCount = 0;
            comboTails.forEach(t => {
              if (prevTails.includes(t)) sameCount++;
              else if (prevTails.some(pt => Math.abs(pt - t) === 1 || Math.abs(pt - t) === 9)) neighborCount++;
            ...
            const changeKey = `same${sameCount}_neighbor${neighborCount}_far${farCount}`;
            const freq = targetPatterns.tailChangeFreq.get(changeKey) || 0;
            if (freq >= 3) tailRelationBonus += 5;
            else if (freq >= 2) tailRelationBonus += 3;
            else if (freq >= 1) tailRelationBonus += 1;
          }
        }
      }
      
      // 综合评分：覆盖数 × 50 + 原始分数 + 尾号关系加分 + 尾号多样性加分
      const combined = newCoverage * 50 + c.score + tailRelationBonus + tailDiversityBonus;
...
function updatePredictionDisplay(predictedTails, ivPrediction, firstBallPredictions, sourceRow, selectedNumbers, _preCollected, _candidateEntries) {
  ...
  if (predictedTails && predictedTails.length > 0) {
    ...
    tailPredictionDiv.innerHTML = `
      ...
      <div class="prediction-info">基于10种尾号模式分析 → 实际号码组合</div>
    `;
  } else {
...
sampleButton.addEventListener("click", () => {
  ...
  if (tailPool.length >= 2) {
    ...
    // 🆕 候选池动态扩展：当候选池太小时，补充历史高频号码（不加邻号尾号）
    ...
    // 🆕 更新预测显示
    updatePredictionDisplay(predictedTails, ivPrediction, firstBallPredictions, sourceRow, selectedNumbers, null, tailEntries);
  ...
  // 获取目标号码尾号模式（关联性、分布规律、时序规律）
  let targetPatterns = null;
  if (isV4) {
    const allBallsForPatterns = collectBalls();
    if (allBallsForPatterns && allBallsForPatterns.length > 0) {
      targetPatterns = analyzeTargetTailPatterns(sourceStartRow, allBallsForPatterns, 100);
    }
  }
  ...
  const variantPlan = isV4
    ? buildV4SingleSamplePlan(frontCombos, backCombos, poolNumbers, frontSample.candidateEntries, frontSample.predictedTails || null, firstBallPreds, targetPatterns, sourceStartRow, allBallsForPatterns, _arithConsecTails, _multiSegTails)
  ...
  if (isV4 && poolNumbers.length > 0) {
    const poolRow = predictionStartRow + 6; // 补漏6是+5，候选池是+6
  ...
  if (isV4) {
    ...
    updatePredictionDisplay(predictedTails, ivPrediction, firstBallPredictions, sourceRow, selectedNumbers, null, frontSample.candidateEntries);
  } else {
...
// 候选池构建方式开关
// true: 只使用预测尾号+邻号构建候选池（实验）
// false: 使用标准评分排序构建候选池（默认）
const USE_TAIL_ONLY_POOL = true;
...
if (__isNode) {
  ...
  for (let sourceIdx = 1; sourceIdx <= totalDraws - PREDICT_INTERVAL - 1; sourceIdx++) {
    ...
    // 3源融合配置（N+9/N/N-1）
    const mainSourceIdx = sourceIdx + SECOND_INTERVAL; // N+9期（主源）
    const auxSourceIdx1 = sourceIdx + THIRD_INTERVAL;  // N期（辅源1）
    const auxSourceIdx2 = sourceIdx + FOURTH_INTERVAL; // N-1期（辅源2）
    ...
    // 收集所有候选源（3源融合：主源N+9期，辅源N期，辅源N-1期）
    const secondSourceRow = auxSourceIdx1 + 1; // N期行号
    const thirdSourceRow = auxSourceIdx2 + 1;  // N-1期行号
    ...
    try {
      ...
      const allSamples = sourceRows.map(row => ({
        row,
        sample: USE_TAIL_ONLY_POOL
          ? buildSampleNumbersV4TailOnly(row, "front", ratioPlan, __allBalls)
          : buildSampleNumbersV4(row, "front", ratioPlan, __allBalls)
      }));
      ...
      // 诊断：记录每个源池的完整35球排名
      const _diagSourceAll35 = allSamples.map(item => {
        const all = (item.sample._allCandidates || []).map((c, i) => ({ num: c.number, score: c.score }));
        return all;
      });
      
      // 3. 3个源各自独立生成组合（辅源使用主源的尾号预测）
      const weights = [0.5, 0.3, 0.2];
      const mainPredictedTails = allSamples[0].sample.predictedTails || null; // 主源的尾号预测
      const allSourceCombos = allSamples.map((item, idx) => {
        const s = item.sample;
        const v4Refs = s.referenceRows || [];
        // 辅源使用主源的尾号预测，避免三源收敛到相同预测
      ...
      // 🆕 诊断：统计三个源的predictedTails一致性
      const allPredTails = allSourceCombos.map(item => {
        const pt = item.sample.predictedTails || [];
        return pt.slice(0, 5).map(([t]) => t);
      });
      if (allPredTails.every(pt => pt.length > 0)) {
        // 统计Top5尾号的一致性
        const tailFreq = new Map();
        for (let t = 0; t <= 9; t++) tailFreq.set(t, 0);
        allPredTails.forEach(pt => pt.forEach(t => tailFreq.set(t, tailFreq.get(t) + 1)));
        const consensusTails = [...tailFreq.entries()].filter(([, c]) => c >= 2).map(([t]) => t);
        const uniqueTails = [...tailFreq.entries()].filter(([, c]) => c === 1).map(([t]) => t);
        console.log(`[尾号一致性] 主源行${sourceRow} Top5: ${allPredTails[0].join(',')}`);
        console.log(`[尾号一致性] 辅源行${secondSourceRow} Top5: ${allPredTails[1].join(',')}`);
        console.log(`[尾号一致性] 辅源行${thirdSourceRow} Top5: ${allPredTails[2].join(',')}`);
        console.log(`[尾号一致性] 共识尾号(≥2源): ${consensusTails.join(',') || '无'}`);
        console.log(`[尾号一致性] 独立尾号(1源): ${uniqueTails.join(',') || '无'}`);
      }
      
      // 4. 组合阶段融合：合并所有组合，按加权分数排序
      const allCombos = [];
      ...
      // 5. 候选池融合（用于覆盖统计和补漏6生成）
      const weightsMap = new Map();
      weights.forEach((w, i) => weightsMap.set(sourceRows[i], w));
      ...
      // 🆕 邻号扩展：为池中每个号码添加±1邻号，提高覆盖率
      const neighborExpanded = new Set(poolNumbers);
      for (const n of poolNumbers) {
        if (n > 1) neighborExpanded.add(n - 1);
        if (n < 35) neighborExpanded.add(n + 1);
      }
      const expandedPoolNumbers = [...neighborExpanded].sort((a, b) => a - b);
      
      // 诊断：保存完整排序列表（不截断）
      const _diagMergedSorted = [...scoreMap.entries()].sort((a, b) => b[1] - a[1]);
      ...
      const predictedTails = [...tailScores.entries()]
        .sort((a, b) => b[1] - a[1])
        .slice(0, 10);
      
      // 定义frontSample对象（用于后续指标计算）
      const frontSample = {
        candidates: expandedPoolNumbers,  // 🆕 使用邻号扩展后的候选池
        candidateEntries: mergedEntries,
        numbers: poolNumbers.slice(0, 5),
        selectedNumbers: mainSample.selectedNumbers,
        predictedTails: predictedTails,
        ivPrediction: ivPrediction,
        referenceRows: [...new Set(allSamples.flatMap(s => s.sample.referenceRows || []))].sort((a, b) => a - b),
        firstBallPredictions: mainSample.firstBallPredictions,
        extremeFlags: mainSample.extremeFlags,
      };
      
      // 7. 后区生成
      const backSample = buildSampleNumbers(sourceRow, "back", [], __allBalls);
      ...
      // 8. 覆盖优先策略选择Top5
      // 获取目标号码尾号模式（关联性、分布规律、时序规律）
      const targetPatterns = analyzeTargetTailPatterns(sourceRow, __allBalls, 100);
      const mainSourceTails = mainSample.selectedNumbers ? [...new Set(mainSample.selectedNumbers.map(n => n % 10))] : [];
      // 获取前一期号码尾号
      const prevRow = sourceRow - 1;
      const prevBalls = __allBalls.filter(b => b.zone === "front" && b.row === prevRow);
      const prevTails = prevBalls.length > 0 ? [...new Set(prevBalls.map(b => b.number % 10))] : [];
      ...
      // 9. 生成完整方案（前区+后区）
      const variantPlan = selectedFront.map((item, idx) => ({
        key: item.key || `combo-${idx + 1}`,
        label: item.label || `第${idx + 1}行`,
        front: item.front || item,
        back: item.back || backCombos[idx % backCombos.length] || backCombos[0],
        title: item.title || [`源${sourceRow}`, item.front?.ratioText ? `前区 ${item.front.ratioText}` : "", "后区自由"].filter(Boolean).join(" / "),
      }));
      if (mergedEntries.length > 0 && selectedFront.length >= 1) {
        const top5Covered = new Set();
        selectedFront.forEach(c => (c.numbers || []).forEach(n => top5Covered.add(n)));
        const top5Freq = new Map();
        selectedFront.forEach(c => (c.numbers || []).forEach(n => top5Freq.set(n, (top5Freq.get(n) || 0) + 1)));
        ...
        const c6Scored = mergedEntries
          ...
          .sort((a, b) => b.c6Score - a.c6Score);
        
        if (c6Scored.length >= 5) {
          const bl6Nums = c6Scored.slice(0, 5).map(e => e.number).sort((a, b) => a - b);
          const bl6Back = backCombos[5 % backCombos.length] || backCombos[0];
          variantPlan.push({
            key: 'supplement-6',
            label: '补漏6',
            front: { numbers: bl6Nums, score: 0, ratioText: '补漏' },
            back: bl6Back,
            title: '补漏6',
          });
        }
      ...
      // ─── 提取结果 ───
      // Top6 = variantPlan 的前6个前区组合（双模式）
      const top6 = variantPlan.slice(0, 6).map(v => ({
      ...
      // 补漏6 = variantPlan 的第6个（索引5，如果存在）
      const bl6 = variantPlan.length > 5 ? {
        numbers: (variantPlan[5]?.front?.numbers || []).slice(0, 5).sort((a, b) => a - b),
      } : null;
      
      // ─── 收集预测详情 ───
      const issue = targetDraw.issue; // 期号
      const sourceNumsStr = sourceNums.join(','); // 源号码
      const targetNumsStr = targetNums.join(','); // 目标号码
      const modeLabels = ['等差①', '等差②', '等差③', '多段①', '多段②', '多段③'];
      
      // 收集Top6
      for (let i = 0; i < Math.min(6, top6.length); i++) {
        const frontNums = top6[i].numbers || [];
        const backNums = variantPlan[i]?.back?.numbers || [];
        predictionDetails.push({
          issue,
          sourceNums: sourceNumsStr,
          targetNums: targetNumsStr,
          predictionType: modeLabels[i] || `Top${i+1}`,
          front1: frontNums[0] || '',
          front2: frontNums[1] || '',
          front3: frontNums[2] || '',
          front4: frontNums[3] || '',
      ...
      // 候选池（使用V4_POOL_SIZE配置 + 补充球）
      const pool30 = new Set(frontSample.candidates || []);
      ...
      // Top6指标（双模式）
      const top6Hits = top6.map(c => c.numbers.filter(n => targetSet.has(n)).length);
      const top6Union = new Set();
      top6.forEach(c => c.numbers.forEach(n => top6Union.add(n)));
      const top6UnionCoverage = targetNums.filter(n => top6Union.has(n)).length;
      ...
      const predictedTailsList = predictedTails || [];
      const tailHitStats = {};
      for (let topN = 1; topN <= 8; topN++) {
        const predTailSet = new Set(predictedTailsList.slice(0, topN).map(([t]) => t));
        const hitCount = targetTails.filter(t => predTailSet.has(t)).length;
        tailHitStats[`top${topN}`] = {
          predicted: [...predTailSet],
          hitCount,
          hitRate: hitCount / Math.min(topN, targetTails.length),
          coverage: hitCount / targetTails.length
        };
      }
      
      // Top5 每注详细信息（含号码 + 命中号码）
      const top5Details = top5.map(c => {
        const hitNums = c.numbers.filter(n => targetSet.has(n));
        return { numbers: c.numbers, hitCount: hitNums.length, hitNumbers: hitNums };
      });
      // Top6详细信息（双模式）
      const top6Details = top6.map(c => {
        const hitNums = c.numbers.filter(n => targetSet.has(n));
        return { numbers: c.numbers, hitCount: hitNums.length, hitNumbers: hitNums };
      });
      // 补漏6 详细信息（如果存在）
      ...
      // 区间比分析
      const poolArr = [...pool30];
      const poolNums = poolArr.sort((a, b) => a - b);
      const poolIv = [
        poolArr.filter(n => n >= 1 && n <= 12).length,
        poolArr.filter(n => n >= 13 && n <= 24).length,
        poolArr.filter(n => n >= 25 && n <= 35).length,
      ];
      const targetIv = [
        targetNums.filter(n => n >= 1 && n <= 12).length,
        targetNums.filter(n => n >= 13 && n <= 24).length,
        targetNums.filter(n => n >= 25 && n <= 35).length,
      ];
      const predIv = frontSample.ivPrediction ? frontSample.ivPrediction.predictedIv : null;
      
      // 🆕 诊断：追踪未覆盖球的排名和源池信息
      const _missedBalls = targetNums.filter(n => !pool30.has(n));
      const _diagMissed = _missedBalls.map(n => {
        const rankInMerged = _diagMergedSorted.findIndex(([num]) => num === n) + 1;
        const ballScore = scoreMap.get(n) || 0;
        const cutoffScore = _diagMergedSorted.length >= V4_POOL_SIZE ? _diagMergedSorted[V4_POOL_SIZE - 1][1] : 0;
        // 在每个源池中的排名（1-35，0=不在池中）
        const srcRanks = _diagSourceAll35.map(all => {
          const idx = all.findIndex(e => e.num === n);
          return idx >= 0 ? idx + 1 : 0;
        });
        // 在每个源的Top30中？在完整35球中？
        const inTop30 = srcRanks.map(r => r > 0 && r <= 30);
        const inAll35 = srcRanks.map(r => r > 0);
        return { num: n, rank: rankInMerged, score: ballScore, gap: ballScore - cutoffScore, srcRanks, inTop30, inAll35 };
      });
      ...
      results.push({
        ...
        top6Hits, top6UnionCoverage,
        bl6Hits, bl6UnionCoverage,
        poolCoverage,
        poolSize: pool30.size,
        totalCombos: allCombos.length,
        poolNums: poolArr.sort((a, b) => a - b),
        missedBalls: _missedBalls,
        diagMissed: _diagMissed,
        poolIv, targetIv, predIv,
        top5Details, top6Details, bl6Detail, poolHitNums,
        tailHitStats, targetTails, predictedTails: predictedTailsList,
      });
      ...
      results.push({
        ...
        top5Details: null, top6Details: null, bl6Detail: null, poolHitNums: [],
        error: err.message,
      });
    }
  }
  ...
  // ═══ 输出结果 ═══
  let sumTop5Max = 0, sumTop5Union = 0, sumBl6Hits = 0, sumUnion = 0, sumPool = 0, sumTop6Union = 0;
  let sumAll7Union = 0; // 7组联合覆盖
  let cnt = 0;
  const csvRows = ['期数,目标号码,Top5①,Top5②,Top5③,Top5④,Top5⑤,Top6①,Top6②,Top6③,Top6④,Top6⑤,Top6⑥,Top5联合覆盖,Top6联合覆盖,候选池覆盖'];
  ...
  results.forEach(r => {
    ...
    sumTop6Union += r.top6UnionCoverage;
    sumBl6Hits += r.bl6Hits;
    sumUnion += r.bl6UnionCoverage;
    sumPool += r.poolCoverage;
    
    // Collect CSV data（Top5 vs Top6 对比）
    const t5 = r.top5Hits || [0,0,0,0,0];
    const t6 = r.top6Hits || [0,0,0,0,0,0];
    csvRows.push(`${cnt},"${r.targetNums.join(' ')}",${t5[0]},${t5[1]},${t5[2]},${t5[3]},${t5[4]},${t6[0]},${t6[1]},${t6[2]},${t6[3]},${t6[4]},${t6[5]},${r.top5UnionCoverage},${r.top6UnionCoverage},${r.poolCoverage}`);
    ...
    // 尾号预测详情
    if (r.predictedTails && r.targetTails) {
      const predTails = r.predictedTails.slice(0, 5).map(([t]) => t);
      const targetTails = r.targetTails;
      const hitTails = targetTails.filter(t => predTails.includes(t));
      
      console.log('  【指标0】尾号预测详情:');
      console.log(`    预测尾号(Top5): [${predTails.join(', ')}]`);
      console.log(`    实际尾号:       [${targetTails.join(', ')}]`);
      console.log(`    命中尾号:       [${hitTails.join(', ')}]`);
      console.log(`    尾号命中率:     ${hitTails.length}/${targetTails.length} = ${(hitTails.length / targetTails.length * 100).toFixed(1)}%`);
      console.log('');
    }
    
    // 指标1: Top5 vs Top6 每注命中对比
    console.log('  【指标1】Top5 vs Top6 每注命中对比:');
    const modeLabels = ['等差①', '等差②', '等差③', '多段①', '多段②', '多段③'];
    if (r.top6Details) {
      r.top6Details.forEach((t, idx) => {
        const mark = t.numbers.map(n => t.hitNumbers.includes(n) ? `*${n}*` : String(n).padStart(2)).join(', ');
        const label = modeLabels[idx] || `Top${idx+1}`;
        const inTop5 = idx < 5 ? '✓' : '✗';
        console.log(`    ${label}: [${mark}] → 命中 ${t.hitCount} 个 ${inTop5}`);
      });
    }
    console.log('');
    
    // 指标2: 联合覆盖对比
    console.log('  【指标2】联合覆盖对比:');
    console.log(`    Top5 联合覆盖(双模式前5): ${r.top5UnionCoverage} / 5`);
    console.log(`    Top6 联合覆盖(双模式6组): ${r.top6UnionCoverage} / 5`);
    console.log(`    候选池覆盖: ${r.poolCoverage} / 5`);
    console.log('');
  });
  
  console.log("");
  console.log("=".repeat(90));
  console.log("汇总统计（Top5 vs Top6 对比）");
  console.log("=".repeat(90));
  console.log(`  总验证期数: ${cnt}`);
  console.log(`  平均 Top5 联合覆盖: ${(sumTop5Union / cnt).toFixed(2)} / 5`);
  console.log(`  平均 Top6 联合覆盖: ${(sumTop6Union / cnt).toFixed(2)} / 5`);
  console.log(`  平均 候选池覆盖: ${(sumPool / cnt).toFixed(2)} / 5`);
  
  // 命中率
  const top5UnionRate = (sumTop5Union / (cnt * 5) * 100).toFixed(1);
  const top6UnionRate = (sumTop6Union / (cnt * 5) * 100).toFixed(1);
  const poolRate = (sumPool / (cnt * 5) * 100).toFixed(1);
  console.log("");
  console.log(`  Top5联合覆盖率: ${top5UnionRate}%`);
  console.log(`  Top6联合覆盖率: ${top6UnionRate}%`);
  console.log(`  候选池覆盖率: ${poolRate}%`);
  
  // 提升幅度
  const improvement = (parseFloat(top6UnionRate) - parseFloat(top5UnionRate)).toFixed(1);
  console.log("");
  console.log(`  Top6 vs Top5 提升: ${improvement > 0 ? '+' : ''}${improvement}%`);
  
  // 🆕 尾号预测命中率统计
  console.log("");
  console.log("=".repeat(90));
  console.log("尾号预测命中率统计");
  console.log("=".repeat(90));
  
  const tailHitSummary = {};
  for (let topN = 1; topN <= 8; topN++) {
    tailHitSummary[`top${topN}`] = { totalHit: 0, totalPredicted: 0, totalTarget: 0 };
  }
  
  results.forEach(r => {
    if (!r.tailHitStats) return;
    const targetTailCount = r.targetTails ? r.targetTails.length : 0;
    for (let topN = 1; topN <= 8; topN++) {
      const stats = r.tailHitStats[`top${topN}`];
      if (stats) {
        tailHitSummary[`top${topN}`].totalHit += stats.hitCount;
        tailHitSummary[`top${topN}`].totalPredicted += topN;
        tailHitSummary[`top${topN}`].totalTarget += targetTailCount;
      }
    }
  });
  
  console.log("\n  尾号预测TopN命中率（命中数/预测数）:");
  console.log("  TopN | 命中总数 | 预测总数 | 目标总数 | 命中率 | 覆盖率");
  console.log("  " + "-".repeat(65));
  
  for (let topN = 1; topN <= 8; topN++) {
    const s = tailHitSummary[`top${topN}`];
    const hitRate = (s.totalHit / s.totalPredicted * 100).toFixed(1);
    const coverageRate = (s.totalHit / s.totalTarget * 100).toFixed(1);
    console.log(`  Top${topN}  | ${String(s.totalHit).padStart(6)} | ${String(s.totalPredicted).padStart(6)} | ${String(s.totalTarget).padStart(6)} | ${hitRate.padStart(5)}% | ${coverageRate.padStart(5)}%`);
  }
  ...
  // ═══ 动态池大小统计 ═══
  const poolSizes = results.map(r => r.poolSize);
  const avgPoolSize = (poolSizes.reduce((a, b) => a + b, 0) / poolSizes.length).toFixed(1);
  const minPoolSize = Math.min(...poolSizes);
  const maxPoolSize = Math.max(...poolSizes);
  const poolSizeDist = {};
  poolSizes.forEach(s => { poolSizeDist[s] = (poolSizeDist[s] || 0) + 1; });
  
  console.log("");
  console.log("=".repeat(90));
  console.log("动态候选池大小统计");
  console.log("=".repeat(90));
  console.log(`  平均池大小: ${avgPoolSize}球`);
  console.log(`  最小池大小: ${minPoolSize}球`);
  console.log(`  最大池大小: ${maxPoolSize}球`);
  console.log("");
  console.log("  池大小分布:");
  const sortedSizes = Object.keys(poolSizeDist).map(Number).sort((a, b) => a - b);
  for (const s of sortedSizes) {
    console.log(`    ${s}球: ${poolSizeDist[s]}次 (${(poolSizeDist[s] / cnt * 100).toFixed(1)}%)`);
  }
  ...
  // 候选池区间分布统计
  let poolZone1 = 0, poolZone2 = 0, poolZone3 = 0;
  let targetZone1 = 0, targetZone2 = 0, targetZone3 = 0;
  let predIvSum = [0, 0, 0];
  let predIvCount = 0;
  
  results.forEach(r => {
    poolZone1 += r.poolIv[0];
    poolZone2 += r.poolIv[1];
    poolZone3 += r.poolIv[2];
    targetZone1 += r.targetIv[0];
    targetZone2 += r.targetIv[1];
    targetZone3 += r.targetIv[2];
    if (r.predIv) {
      predIvSum[0] += r.predIv[0];
      predIvSum[1] += r.predIv[1];
      predIvSum[2] += r.predIv[2];
      predIvCount++;
    }
  });
  
  const totalPool = poolZone1 + poolZone2 + poolZone3;
  const totalTarget = targetZone1 + targetZone2 + targetZone3;
  
  console.log(`  候选池区间分布 (总计${totalPool}球):`);
  console.log(`    一区(1-12):  ${poolZone1}球 (${(poolZone1 / totalPool * 100).toFixed(1)}%)  平均${(poolZone1 / cnt).toFixed(1)}球/期`);
  console.log(`    二区(13-24): ${poolZone2}球 (${(poolZone2 / totalPool * 100).toFixed(1)}%)  平均${(poolZone2 / cnt).toFixed(1)}球/期`);
  console.log(`    三区(25-35): ${poolZone3}球 (${(poolZone3 / totalPool * 100).toFixed(1)}%)  平均${(poolZone3 / cnt).toFixed(1)}球/期`);
  
  console.log(`  目标号码区间分布 (总计${totalTarget}球):`);
  console.log(`    一区(1-12):  ${targetZone1}球 (${(targetZone1 / totalTarget * 100).toFixed(1)}%)  平均${(targetZone1 / cnt).toFixed(1)}球/期`);
  console.log(`    二区(13-24): ${targetZone2}球 (${(targetZone2 / totalTarget * 100).toFixed(1)}%)  平均${(targetZone2 / cnt).toFixed(1)}球/期`);
  console.log(`    三区(25-35): ${targetZone3}球 (${(targetZone3 / totalTarget * 100).toFixed(1)}%)  平均${(targetZone3 / cnt).toFixed(1)}球/期`);
  
  if (predIvCount > 0) {
    console.log(`  预测区间比 (平均, ${predIvCount}期):`);
    console.log(`    一区: ${(predIvSum[0] / predIvCount).toFixed(2)}  二区: ${(predIvSum[1] / predIvCount).toFixed(2)}  三区: ${(predIvSum[2] / predIvCount).toFixed(2)}`);
  }
  
  // 区间覆盖率
  const zone1Cov = targetZone1 > 0 ? ((targetZone1 - results.reduce((s, r) => s + Math.max(0, r.targetIv[0] - r.poolIv[0]), 0)) / targetZone1 * 100).toFixed(1) : 'N/A';
  // 更精确：逐期计算每个区间的覆盖
  let z1Hit = 0, z2Hit = 0, z3Hit = 0;
  results.forEach(r => {
    const pool = new Set(r.poolNums);
    r.targetNums.forEach(n => {
      if (pool.has(n)) {
        if (n <= 12) z1Hit++;
        else if (n <= 24) z2Hit++;
        else z3Hit++;
  ...
  console.log(`  分区间覆盖率:`);
  console.log(`    一区: ${z1Hit}/${targetZone1} = ${(z1Hit / targetZone1 * 100).toFixed(1)}%`);
  console.log(`    二区: ${z2Hit}/${targetZone2} = ${(z2Hit / targetZone2 * 100).toFixed(1)}%`);
  console.log(`    三区: ${z3Hit}/${targetZone3} = ${(z3Hit / targetZone3 * 100).toFixed(1)}%`);
  
  // 预测区间比 vs 实际区间比的偏差
  const predIvDist = {};
  results.forEach(r => {
    if (r.predIv) {
      const key = r.predIv.join(":");
      predIvDist[key] = (predIvDist[key] || 0) + 1;
    }
  });
  console.log(`  预测区间比分布 (Top 10):`);
  Object.entries(predIvDist)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10)
    .forEach(([iv, count]) => {
      console.log(`    ${iv}: ${count}次 (${(count / cnt * 100).toFixed(1)}%)`);
    });
  
  // ═══ 分时段分析 ═══
  console.log("");
  console.log("=".repeat(90));
  console.log("分时段分析（早期 vs 近期）");
  console.log("=".repeat(90));
  
  const segSize = Math.floor(cnt / 3);
  const segments = [
    { name: "早期(前1/3)", start: 0, end: segSize },
    { name: "中期(中1/3)", start: segSize, end: segSize * 2 },
    { name: "近期(后1/3)", start: segSize * 2, end: cnt },
  ];
  
  segments.forEach(seg => {
    const segResults = results.slice(seg.start, seg.end);
    const n = segResults.length;
    if (n === 0) return;
    
    let sTop5Max = 0, sTop5Union = 0, sBl6 = 0, sUnion = 0, sPool = 0;
    let sHit3plus = 0, sUnion4plus = 0, sPool4plus = 0;
    
    segResults.forEach(r => {
      const maxH = Math.max(...r.top5Hits);
      sTop5Max += maxH;
      sTop5Union += r.top5UnionCoverage;
      sBl6 += r.bl6Hits;
      sUnion += r.bl6UnionCoverage;
      sPool += r.poolCoverage;
      if (maxH >= 3) sHit3plus++;
      if (r.bl6UnionCoverage >= 4) sUnion4plus++;
      if (r.poolCoverage >= 4) sPool4plus++;
    });
    
    console.log(`  ${seg.name} (${n}期):`);
    console.log(`    Top5命中率: ${(sTop5Max / (n * 5) * 100).toFixed(1)}%  平均最高命中: ${(sTop5Max / n).toFixed(2)}`);
    console.log(`    Top5+补漏6联合覆盖: ${(sUnion / (n * 5) * 100).toFixed(1)}%  平均: ${(sUnion / n).toFixed(2)}`);
    console.log(`    候选池覆盖: ${(sPool / (n * 5) * 100).toFixed(1)}%  平均: ${(sPool / n).toFixed(2)}`);
    console.log(`    Top5>=3球: ${sHit3plus}次(${(sHit3plus / n * 100).toFixed(1)}%)  联合>=4球: ${sUnion4plus}次(${(sUnion4plus / n * 100).toFixed(1)}%)  池>=4球: ${sPool4plus}次(${(sPool4plus / n * 100).toFixed(1)}%)`);
  });
  
  // ═══ 候选池大小敏感性分析 ═══
  console.log("");
  console.log("=".repeat(90));
  console.log("候选池大小敏感性分析");
  console.log("=".repeat(90));
  
  [20, 22, 24, 26, 28, 30].forEach(poolSize => {
    let pCover = 0, p4plus = 0;
    results.forEach(r => {
      // 用候选池号码的前poolSize个计算覆盖
      const pool = new Set(r.poolNums ? r.poolNums.slice(0, poolSize) : []);
      const cov = r.targetNums.filter(n => pool.has(n)).length;
      pCover += cov;
      if (cov >= 4) p4plus++;
    });
    console.log(`  池大小${poolSize}: 覆盖率=${(pCover / (cnt * 5) * 100).toFixed(1)}%  平均=${(pCover / cnt).toFixed(2)}  >=4球:${p4plus}次(${(p4plus / cnt * 100).toFixed(1)}%)`);
  });
  ...
  console.log("");
  console.log("=".repeat(80));
  console.log("未覆盖球诊断分析（排名=融合后排名，分差=与第30名的分差）");
  console.log("=".repeat(80));
  
  const diagBallStats = new Map(); // ball -> { count, ranks, gaps, inTop30, inAll35 }
  for (const r of results) {
    if (!r.diagMissed) continue;
    for (const dm of r.diagMissed) {
      if (!diagBallStats.has(dm.num)) diagBallStats.set(dm.num, { count: 0, ranks: [], gaps: [], in0: 0, in1: 0, in2: 0, inNone: 0, notIn35_0: 0, notIn35_1: 0, notIn35_2: 0, neverIn35: 0, srcRanksAll: [] });
      const s = diagBallStats.get(dm.num);
      s.count++;
      s.ranks.push(dm.rank);
      s.gaps.push(dm.gap);
      s.srcRanksAll.push(dm.srcRanks);
      if (dm.inTop30[0]) s.in0++;
      if (dm.inTop30[1]) s.in1++;
      if (dm.inTop30[2]) s.in2++;
      if (!dm.inTop30[0] && !dm.inTop30[1] && !dm.inTop30[2]) s.inNone++;
      // 统计完整35球中的情况
      if (!dm.inAll35[0]) s.notIn35_0++;
      if (!dm.inAll35[1]) s.notIn35_1++;
      if (!dm.inAll35[2]) s.notIn35_2++;
      if (!dm.inAll35[0] && !dm.inAll35[1] && !dm.inAll35[2]) s.neverIn35++;
    }
  }
  
  const sortedDiag = [...diagBallStats.entries()].sort((a, b) => b[1].count - a[1].count);
  
  console.log("\n球号 | 漏球次数 | 平均排名 | 排名范围 | 平均分差 | Top30出现(主/辅1/辅2/全无) | 从未入35球");
  console.log("-".repeat(105));
  
  let totalNeverInPool = 0, totalInPoolFiltered = 0;
  let rank31_33 = 0, rank34_35 = 0, rank36plus = 0, rankBelow30 = 0;
  
  for (const [ball, s] of sortedDiag) {
    const avgRank = (s.ranks.reduce((a, b) => a + b, 0) / s.ranks.length).toFixed(1);
    const minRank = Math.min(...s.ranks);
    const maxRank = Math.max(...s.ranks);
    const avgGap = (s.gaps.reduce((a, b) => a + b, 0) / s.gaps.length).toFixed(1);
    
    console.log(
      `球${String(ball).padStart(2)} | ${String(s.count).padStart(4)}次   | ` +
      `${String(avgRank).padStart(6)}  | ${String(minRank).padStart(3)}-${String(maxRank).padStart(3)}   | ` +
      `${String(avgGap).padStart(7)} | ${s.in0}/${s.in1}/${s.in2}/${s.inNone}       | ${s.neverIn35}/${s.count}`
    );
    
    if (s.inNone === s.count) totalNeverInPool += s.count;
    else totalInPoolFiltered += s.count;
    
    for (const r of s.ranks) {
      if (r <= 30) rankBelow30++;
      else if (r <= 33) rank31_33++;
      else if (r <= 35) rank34_35++;
      else rank36plus++;
    }
  }
  
  const _totalMissedDiag = totalNeverInPool + totalInPoolFiltered;
  // 统计完整35球追踪
  let totalNeverIn35 = 0, totalIn35ButNotTop30 = 0;
  for (const [, s] of sortedDiag) {
    totalNeverIn35 += s.neverIn35;
    // 在35球中但不在Top30中的次数
    for (let i = 0; i < s.count; i++) {
      const inAny35 = s.srcRanksAll[i].some(r => r > 0);
      const inAnyTop30 = s.srcRanksAll[i].some(r => r > 0 && r <= 30);
      if (inAny35 && !inAnyTop30) totalIn35ButNotTop30++;
    }
  }
  console.log("\n" + "=".repeat(80));
  console.log("分类统计:");
  console.log(`  从未进入任何源池(Top30): ${totalNeverInPool}次 (${(totalNeverInPool/Math.max(1,_totalMissedDiag)*100).toFixed(1)}%)`);
  console.log(`  进入源池但融合后被筛掉: ${totalInPoolFiltered}次 (${(totalInPoolFiltered/Math.max(1,_totalMissedDiag)*100).toFixed(1)}%)`);
  console.log(`  融合后排名分布: <=30(应在池中):${rankBelow30}次, 31-33:${rank31_33}次, 34-35:${rank34_35}次, 36+:${rank36plus}次`);
  console.log(`\n  完整35球追踪:`);
  console.log(`    从未进入任何源35球: ${totalNeverIn35}次 (${(totalNeverIn35/Math.max(1,_totalMissedDiag)*100).toFixed(1)}%)`);
  console.log(`    在35球中但排名31-35被截断: ${totalIn35ButNotTop30}次 (${(totalIn35ButNotTop30/Math.max(1,_totalMissedDiag)*100).toFixed(1)}%)`);
  console.log(`    在Top30但融合加权后被筛掉: ${_totalMissedDiag - totalNeverIn35 - totalIn35ButNotTop30}次 (${((_totalMissedDiag - totalNeverIn35 - totalIn35ButNotTop30)/Math.max(1,_totalMissedDiag)*100).toFixed(1)}%)`);
  
  // 分区间统计
  const zones = [
    { name: "一区(1-12)", min: 1, max: 12 },
    { name: "二区(13-24)", min: 13, max: 24 },
    { name: "三区(25-35)", min: 25, max: 35 },
  ];
  console.log("\n分区间统计:");
  for (const z of zones) {
    const zBalls = sortedDiag.filter(([b]) => b >= z.min && b <= z.max);
    const zTotal = zBalls.reduce((s, [, st]) => s + st.count, 0);
    const zNever = zBalls.reduce((s, [, st]) => s + st.inNone, 0);
    const zNeverIn35 = zBalls.reduce((s, [, st]) => s + st.neverIn35, 0);
    const zFiltered = zTotal - zNever;
    console.log(`  ${z.name}: 总漏${zTotal}次, 从未入Top30:${zNever}(${(zNever/Math.max(1,zTotal)*100).toFixed(0)}%), 入池被筛:${zFiltered}(${(zFiltered/Math.max(1,zTotal)*100).toFixed(0)}%), 从未入35球:${zNeverIn35}(${(zNeverIn35/Math.max(1,zTotal)*100).toFixed(0)}%)`);
  }
  
  console.log("");
  console.log("完成!");
  
  // Write per-period CSV
  const csvPath = 'per_period_detail.csv';
  require('fs').writeFileSync(csvPath, csvRows.join('\n'), 'utf-8');
  console.log(`已保存每期详细数据到: ${csvPath}`);
  
  // 生成预测详情Excel文件
  if (__isNode && process.argv.includes('--generate-excel')) {
    const csvPath2 = 'predictions_detail_v6.csv';
    const csvRows2 = ['期号,源号码,目标号码,预测类型,前区1,前区2,前区3,前区4,前区5,后区1,后区2'];
    ...
    require('fs').writeFileSync(csvPath2, csvRows2.join('\n'), 'utf-8');
    console.log(`已保存预测详情到: ${csvPath2}`);
    console.log('使用Python脚本将CSV转换为Excel: python convert_csv_to_excel.py predictions_detail_v6.csv predictions_detail_v6.xlsx');
  }
}

// ======================== 固定种子 vs 随机种子对比实验 ========================
if (__isNode && process.argv.includes('--compare-seeds')) {
  console.log("\n\n");
  console.log("=".repeat(90));
  ...
  // 自定义种子随机数生成器
  function makeSeededRandom(seedVal) {
    let s = seedVal;
    return function() {
      s |= 0;
      s = s + 0x6D2B79F5 | 0;
  ...
  console.log("[调试] 即将定义 runQuickBacktest 函数");
  // 运行单次回测的简化版（只统计核心指标）
  function runQuickBacktest(useFixedSeed, seedVal) {
    console.log("[调试] runQuickBacktest 开始执行, useFixedSeed:", useFixedSeed, "seedVal:", seedVal);
    if (useFixedSeed) {
      Math.random = makeSeededRandom(seedVal);
    } else {
    ...
    for (let sourceIdx = 1; sourceIdx <= totalDraws - PREDICT_INTERVAL - 1; sourceIdx++) {
      ...
      const sourceNums = [...sourceDraw.front].sort((a, b) => a - b);
      console.log(`[调试] sourceNums长度: ${sourceNums.length}, sourceNums: [${sourceNums.join(', ')}]`);
      ...
      try {
        ...
        results.push({ top5Max, top5Union, poolHit });
      } catch (err) {
        // 跳过错误
      }
    }
  ...
  console.log("[调试] runQuickBacktest 函数定义完成");
  
  // 支持--quick参数：快速模式只测试2个种子+1次随机
  const isQuick = process.argv.includes('--quick');
  const seedCount = isQuick ? 2 : 10;
  const randomCount = isQuick ? 1 : 5;
  
  // 测试多个固定种子
  console.log("[调试] 即将开始固定种子测试" + (isQuick ? "（快速模式）" : ""));
  console.log("\n【固定种子测试】");
  console.log("-".repeat(90));
  const allSeeds = [12345, 67890, 11111, 54321, 99999, 123456, 88888, 77777, 33333, 55555];
  const seeds = allSeeds.slice(0, seedCount);
  const seedResults = [];
  
  for (const seed of seeds) {
    const result = runQuickBacktest(true, seed);
    if (result) {
      seedResults.push({ seed, ...result });
      console.log(`种子 ${String(seed).padStart(6)}: Top5=${result.avgTop5Max}, 联合=${result.avgTop5Union}, 池=${result.avgPool}, 3+球=${result.hit3}, 4+球=${result.hit4}, 5球=${result.hit5}`);
    }
  }
  
  // 测试随机种子
  console.log(`\n【随机种子测试】(${randomCount}次运行)`);
  console.log("-".repeat(90));
  const randomResults = [];
  
  for (let i = 0; i < randomCount; i++) {
    const result = runQuickBacktest(false, 0);
    if (result) {
      randomResults.push(result);
      console.log(`随机第${i+1}次: Top5=${result.avgTop5Max}, 联合=${result.avgTop5Union}, 池=${result.avgPool}, 3+球=${result.hit3}, 4+球=${result.hit4}, 5球=${result.hit5}`);
    }
  }
  
  // 计算平均值
  const avgSeed = {
    avgTop5Max: (seedResults.reduce((s, r) => s + parseFloat(r.avgTop5Max), 0) / seedResults.length).toFixed(3),
    avgTop5Union: (seedResults.reduce((s, r) => s + parseFloat(r.avgTop5Union), 0) / seedResults.length).toFixed(3),
    avgPool: (seedResults.reduce((s, r) => s + parseFloat(r.avgPool), 0) / seedResults.length).toFixed(3),
  ...
  const avgRandom = {
    avgTop5Max: (randomResults.reduce((s, r) => s + parseFloat(r.avgTop5Max), 0) / randomResults.length).toFixed(3),
    avgTop5Union: (randomResults.reduce((s, r) => s + parseFloat(r.avgTop5Union), 0) / randomResults.length).toFixed(3),
    avgPool: (randomResults.reduce((s, r) => s + parseFloat(r.avgPool), 0) / randomResults.length).toFixed(3),
  ...
  // 计算标准差
  const seedStdTop5 = Math.sqrt(seedResults.reduce((s, r) => s + Math.pow(parseFloat(r.avgTop5Max) - parseFloat(avgSeed.avgTop5Max), 2), 0) / seedResults.length).toFixed(3);
  const randomStdTop5 = Math.sqrt(randomResults.reduce((s, r) => s + Math.pow(parseFloat(r.avgTop5Max) - parseFloat(avgRandom.avgTop5Max), 2), 0) / randomResults.length).toFixed(3);
  
  console.log("\n" + "=".repeat(90));
  console.log("汇总对比");
  console.log("=".repeat(90));
  console.log("\n指标                    | 固定种子(10个) | 随机种子(5次) | 差异     | 结论");
  console.log("-".repeat(90));
  
  const diff5 = (parseFloat(avgSeed.avgTop5Max) - parseFloat(avgRandom.avgTop5Max)).toFixed(3);
  const diffUnion = (parseFloat(avgSeed.avgTop5Union) - parseFloat(avgRandom.avgTop5Union)).toFixed(3);
  const diffPool = (parseFloat(avgSeed.avgPool) - parseFloat(avgRandom.avgPool)).toFixed(3);
  
  console.log(`Top5最高命中(平均)      | ${avgSeed.avgTop5Max}        | ${avgRandom.avgTop5Max}        | ${diff5 > 0 ? '+' : ''}${diff5}   | ${Math.abs(parseFloat(diff5)) < 0.01 ? '基本相同' : (parseFloat(diff5) > 0 ? '固定略高' : '随机略高')}`);
  console.log(`Top5+补漏6联合覆盖(平均)| ${avgSeed.avgTop5Union}        | ${avgRandom.avgTop5Union}        | ${diffUnion > 0 ? '+' : ''}${diffUnion}   | ${Math.abs(parseFloat(diffUnion)) < 0.01 ? '基本相同' : (parseFloat(diffUnion) > 0 ? '固定略高' : '随机略高')}`);
  console.log(`候选池覆盖(平均)        | ${avgSeed.avgPool}        | ${avgRandom.avgPool}        | ${diffPool > 0 ? '+' : ''}${diffPool}   | ${Math.abs(parseFloat(diffPool)) < 0.01 ? '基本相同' : (parseFloat(diffPool) > 0 ? '固定略高' : '随机略高')}`);
  console.log(`命中3+球期数(平均)      | ${String(avgSeed.hit3).padStart(6)}          | ${String(avgRandom.hit3).padStart(6)}          | ${String(avgSeed.hit3 - avgRandom.hit3).padStart(6)}   |`);
  console.log(`命中4+球期数(平均)      | ${String(avgSeed.hit4).padStart(6)}          | ${String(avgRandom.hit4).padStart(6)}          | ${String(avgSeed.hit4 - avgRandom.hit4).padStart(6)}   |`);
  console.log(`命中5球期数(平均)       | ${String(avgSeed.hit5).padStart(6)}          | ${String(avgRandom.hit5).padStart(6)}          | ${String(avgSeed.hit5 - avgRandom.hit5).padStart(6)}   |`);
  console.log(`Top5标准差              | ${seedStdTop5}         | ${randomStdTop5}         |        |`);
  
  console.log("\n【结论】");
  const absDiff5 = Math.abs(parseFloat(diff5));
  if (absDiff5 < 0.005) {
    console.log("  固定种子和随机种子的命中率几乎没有差异 (< 0.005)");
    console.log("  V5组合生成虽然使用加权随机抽取，但Top5选择基于分数排序（确定性过程）");
    console.log("  因此随机种子对最终推荐结果的影响极小");
  } else if (parseFloat(diff5) > 0) {
    console.log("  固定种子命中率略高，可能是种子值12345恰好适合当前数据特征");
    console.log("  建议：固定种子可确保结果可复现，便于调试和对比");
  } else {
    console.log("  随机种子命中率略高，不同种子值可能产生更好的组合多样性");
    console.log("  建议：使用随机种子可获得略好的预测效果，但结果不可复现");
  }
  
  console.log("\n  推荐方案：");
  console.log("  - 调试/回测时使用固定种子（结果可复现）");
  console.log("  - 实际预测时使用随机种子（略好的组合多样性）");
}