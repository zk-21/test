/**
 * backtest_v5_nopair_enhanced.js — 对比：6维独立 vs 10维独立（+重复/连号/等差/跨期）
 * 
 * A) 原版6维独立：tail→2注, offset→2注, hot→1注, freq→1注, bridge→1注, arith→1注
 * B) 增强10维独立：A的6维 + repeat→1注, run→1注, arithSelf→1注, cross→1注
 * 
 * 新增维度说明：
 * - dimRepeat：重复号码（与上一期重号的加分，历史统计表明重号概率约47%）
 * - dimRun：连号邻接（候选号是否能与其它高分号组成连号序列）
 * - dimArithSelf：等差延伸（候选号是否能与其它高分号组成等差序列）
 * - dimCross：跨期关系（候选号的尾号与上期尾号匹配、与上期号码相邻）
 * 
 * 运行：node backtest_v5_nopair_enhanced.js
 */
...
console.log('对比测试：6维独立(A) vs 10维独立+重复/连号/等差/跨期(B)');
console.log('='.repeat(100));
...
console.log('='.repeat(100));
console.log('');
console.log('指标'.padEnd(25) + 'A)6维独立'.padEnd(15) + 'B)10维增强'.padEnd(15) + '差异'.padEnd(12) + '胜者');
